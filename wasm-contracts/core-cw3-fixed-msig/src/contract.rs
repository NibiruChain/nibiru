...
mod tests {
    ...
    fn test_instantiate_works() {
        ...
        assert_eq!(
            ...
            ContractError::Threshold(
                cw_utils::ThresholdError::InvalidThreshold {}
            )
        );
        ...
        setup_test_case(deps.as_mut(), info, threshold, max_voting_period)
            .unwrap();
    ...
    fn test_propose_works() {
        ...
        let err = execute(deps.as_mut(), mock_env(), info, proposal.clone())
            .unwrap_err();
        assert_eq!(err, ContractError::Unauthorized {});
        ...
        let err = execute(deps.as_mut(), mock_env(), info, proposal_wrong_exp)
            .unwrap_err();
    ...
    fn test_vote_works() {
        ...
        let info = mock_info(OWNER, &[]);
        setup_test_case(deps.as_mut(), info.clone(), threshold, voting_period)
            .unwrap();
        ...
        let err = execute(deps.as_mut(), mock_env(), info, yes_vote.clone())
            .unwrap_err();
        ...
        let err =
            execute(deps.as_mut(), mock_env(), info.clone(), yes_vote.clone())
                .unwrap_err();
        assert_eq!(err, ContractError::AlreadyVoted {});
        assert_eq!(tally, get_tally(deps.as_ref(), proposal_id));
    ...
    fn test_execute_works() {
        ...
        let err = execute(deps.as_mut(), mock_env(), info, execution.clone())
            .unwrap_err();
