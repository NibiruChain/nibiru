...
func GetQueryCmd() *cobra.Command {
	feesQueryCmd := &cobra.Command{
		Use:                        devgas.ModuleName,
		Short:                      fmt.Sprintf("Querying commands for the %s module", devgas.ModuleName),
...
func GetCmdQueryFeeShares() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query %s deployer-contracts <deployer-address>",
			version.AppName, devgas.ModuleName,
		),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := devgas.NewQueryClient(clientCtx)
...
func GetCmdQueryFeeShare() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			req := &devgas.QueryFeeShareRequest{ContractAddress: args[0]}
			if err := req.ValidateBasic(); err != nil {
				return err
			}
...
func GetCmdQueryParams() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			params := &devgas.QueryParamsRequest{}
...
func GetCmdQueryFeeSharesByWithdrawer() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			withdrawReq := &devgas.QueryFeeSharesByWithdrawerRequest{
