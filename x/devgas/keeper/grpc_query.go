...
var _ devgas.QueryServer = Querier{}
...
func (q Querier) FeeShare(
	goCtx context.Context,
	req *devgas.QueryFeeShareRequest,
) (*devgas.QueryFeeShareResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "empty request")
	}
	...
	return &devgas.QueryFeeShareResponse{Feeshare: feeshare}, nil
}

// Params returns the fees module params
func (q Querier) Params(
	c context.Context,
	_ *devgas.QueryParamsRequest,
) (*devgas.QueryParamsResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	params := q.GetParams(ctx)
	return &devgas.QueryParamsResponse{Params: params}, nil
}

// FeeSharesByWithdrawer returns all fees for a given withdraw address
func (q Querier) FeeSharesByWithdrawer( // nolint: dupl
	goCtx context.Context,
	req *devgas.QueryFeeSharesByWithdrawerRequest,
) (*devgas.QueryFeeSharesByWithdrawerResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "empty request")
	}
	ctx := sdk.UnwrapSDKContext(goCtx)
	iter := q.DevGasStore.Indexes.Withdrawer.ExactMatch(ctx, req.WithdrawerAddress)
	return &devgas.QueryFeeSharesByWithdrawerResponse{
		Feeshare: q.DevGasStore.Collect(ctx, iter),
	}, nil
}
