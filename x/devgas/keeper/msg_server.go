...
func (k Keeper) RegisterFeeShare(
	...
	msg *devgas.MsgRegisterFeeShare,
) (*devgas.MsgRegisterFeeShareResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)

	params := k.GetParams(ctx)
	if !params.EnableFeeShare {
		return nil, devgas.ErrFeeShareDisabled
	}
	...
	if k.IsFeeShareRegistered(ctx, contract) {
		return nil, devgas.ErrFeeShareAlreadyRegistered.Wrapf("contract is already registered %s", contract)
	}
	...
	if k.isContractCreatedFromFactory(ctx, k.wasmKeeper.GetContractInfo(ctx, contract), msgSender) {
		...
		if msg.WithdrawerAddress != msg.ContractAddress {
			return nil, devgas.ErrFeeShareInvalidWithdrawer.Wrapf(
	...
	feeshare := devgas.NewFeeShare(contract, deployer, withdrawer)
	...
	return &devgas.MsgRegisterFeeShareResponse{}, ctx.EventManager().EmitTypedEvent(
		&devgas.EventRegisterDevGas{
			Deployer:   msg.DeployerAddress,
			Contract:   msg.ContractAddress,
...
func (k Keeper) UpdateFeeShare(
	goCtx context.Context,
	msg *devgas.MsgUpdateFeeShare,
) (*devgas.MsgUpdateFeeShareResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	params := k.GetParams(ctx)
	if !params.EnableFeeShare {
		return nil, devgas.ErrFeeShareDisabled
	...
	if !found {
		return nil,
			devgas.ErrFeeShareContractNotRegistered.Wrapf(
	...
	if msg.WithdrawerAddress == feeshare.WithdrawerAddress {
		return nil, devgas.ErrFeeShareAlreadyRegistered.Wrapf(
	...
	return &devgas.MsgUpdateFeeShareResponse{}, ctx.EventManager().EmitTypedEvent(
		&devgas.EventUpdateDevGas{
			Deployer:   msg.DeployerAddress,
			Contract:   msg.ContractAddress,
			Withdrawer: msg.WithdrawerAddress,
		},
...
func (k Keeper) CancelFeeShare(
	goCtx context.Context,
	msg *devgas.MsgCancelFeeShare,
) (*devgas.MsgCancelFeeShareResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	...
	if !found {
		return nil, devgas.ErrFeeShareContractNotRegistered.Wrapf(
			"contract %s is not registered", msg.ContractAddress,
		)
	}
	...
	return &devgas.MsgCancelFeeShareResponse{}, ctx.EventManager().EmitTypedEvent(
		&devgas.EventCancelDevGas{
			Deployer: msg.DeployerAddress,
			Contract: msg.ContractAddress,
		},
	)
...
func (k Keeper) UpdateParams(
	goCtx context.Context, req *devgas.MsgUpdateParams,
) (resp *devgas.MsgUpdateParamsResponse, err error) {
	...
	return &devgas.MsgUpdateParamsResponse{}, err
}
