...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	oracleGenesis.ExchangeRates = []oracletypes.ExchangeRateTuple{
		{Pair: common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD), ExchangeRate: sdk.NewDec(20_000)},
		{Pair: common.AssetRegistry.Pair(denoms.DenomETH, denoms.DenomNUSD), ExchangeRate: sdk.NewDec(2_000)},
	}
	...
	s.NoError(
		testutilcli.FillWalletFromValidator(user1,
			sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomNIBI, 10*common.Precision),
				sdk.NewInt64Coin(denoms.DenomUSDC, 10*common.Precision),
	...
	s.NoError(
		testutilcli.FillWalletFromValidator(user2,
			sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomNIBI, 1000),
				sdk.NewInt64Coin(denoms.DenomUSDC, 1000),
				sdk.NewInt64Coin(denoms.DenomNUSD, 100000),
			),
			val,
			denoms.DenomNIBI,
		),
	...
	s.NoError(
		testutilcli.FillWalletFromValidator(user3,
			sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomNIBI, 1000),
				sdk.NewInt64Coin(denoms.DenomUSDC, 1000),
				sdk.NewInt64Coin(denoms.DenomNUSD, 49*common.Precision),
			),
			val,
			denoms.DenomNIBI,
		),
	...
	s.NoError(
		testutilcli.FillWalletFromValidator(user5,
			sdk.NewCoins(sdk.NewInt64Coin(denoms.DenomNIBI, 1000)),
			val,
			denoms.DenomNIBI,
		),
	)
...
func (s *IntegrationTestSuite) TestOpenPositionsAndCloseCmd() {
	...
	exchangeRate, err := testutilcli.QueryOracleExchangeRate(val.ClientCtx, common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD))
	s.T().Logf("0. current exchange rate is: %+v", exchangeRate)
	s.NoError(err)

	s.T().Log("A. check vpool balances")
	reserveAssets, err := testutilcli.QueryVpoolReserveAssets(val.ClientCtx, common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD))
	...
	_, err = testutilcli.QueryPosition(val.ClientCtx, common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD), user)
	s.Error(err)

	s.T().Log("B. open position")
	txResp, err := testutilcli.ExecTx(s.network, cli.OpenPositionCmd(), user, []string{
		"buy",
		common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD).String(),
		/* leverage */ "1",
	...
	queryResp, err := testutilcli.QueryPosition(val.ClientCtx, common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD), user)
	s.NoError(err)
	s.T().Logf("query response: %+v", queryResp)
	s.EqualValues(user.String(), queryResp.Position.TraderAddress)
	s.EqualValues(common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD), queryResp.Position.Pair)
...
func (s *IntegrationTestSuite) TestPositionEmptyAndClose() {
	...
	_, err := testutilcli.QueryPosition(val.ClientCtx, common.AssetRegistry.Pair(denoms.DenomETH, denoms.DenomNUSD), user)
	...
	_, err = testutilcli.ExecTx(s.network, cli.ClosePositionCmd(), user, []string{
		common.AssetRegistry.Pair(denoms.DenomETH, denoms.DenomNUSD).String(),
	})
...
func (s *IntegrationTestSuite) TestQueryCumulativePremiumFractions() {
	...
	queryResp, err := testutilcli.QueryCumulativePremiumFraction(val.ClientCtx, common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD))
	s.NoError(err)
...
func (s *IntegrationTestSuite) TestRemoveMargin() {
	...
	_, err = testutilcli.ExecTx(s.network, cli.RemoveMarginCmd(), s.users[0], []string{
		common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD).String(),
		fmt.Sprintf("%s%s", "100", denoms.DenomNUSD),
	})
