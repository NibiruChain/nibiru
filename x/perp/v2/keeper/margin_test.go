	...
	tc := TestCases{
		TC("existing long position, add margin").
			Given(
				...
				FundAccount(alice, sdk.NewCoins(sdk.NewCoin(types.TestingCollateralDenomNUSD, sdk.NewInt(2020)))),
	...
	tc := TestCases{
		TC("existing long position, remove margin").
			Given(
				...
				SetBlockNumber(1),
				SetBlockTime(startBlockTime),
				FundAccount(alice, sdk.NewCoins(sdk.NewCoin(types.TestingCollateralDenomNUSD, sdk.NewInt(1002)))),
