	...
	tests := TestCases{
		TC("open long position").
			Given(
				...
				FundAccount(alice, sdk.NewCoins(sdk.NewInt64Coin(types.DefaultTestingCollateralNotForProd.GetTFDenom(), 100))),
			).
			When(
			...
			Then(
				...
				BalanceEqual(alice, types.DefaultTestingCollateralNotForProd.GetTFDenom(), sdk.NewInt(99)),
			),
	...
	tests := TestCases{
		TC("close long position").
			...
			Then(
				...
				BalanceEqual(alice, types.DefaultTestingCollateralNotForProd.GetTFDenom(), sdk.NewInt(100)),
			),
	...
	tests := TestCases{
		TC("add margin").
			...
			Then(
				...
				BalanceEqual(alice, types.DefaultTestingCollateralNotForProd.GetTFDenom(), sdk.NewInt(98)),
			),
		TC("partial close").
			Given(
				CreateCustomMarket(pair, WithEnabled(true)),
...
func TestMsgServerDonateToPerpEf(t *testing.T) {
	...
	tests := TestCases{
		TC("success").
			...
			Then(
				BalanceEqual(alice, types.DefaultTestingCollateralNotForProd.GetTFDenom(), sdk.NewInt(50)),
				ModuleBalanceEqual(types.PerpEFModuleAccount, types.DefaultTestingCollateralNotForProd.GetTFDenom(), sdk.NewInt(50)),
			),
	}
