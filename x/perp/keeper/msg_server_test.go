...
func TestMsgServerAddMargin(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			margin:      sdk.NewInt64Coin(denoms.DenomNUSD, 1000),
			expectedErr: sdkerrors.ErrInsufficientFunds,
		},
		{
			name:            "no initial position",
			traderFunds:     sdk.NewCoins(sdk.NewInt64Coin(denoms.DenomNUSD, 1000)),
			initialPosition: nil,
			margin:          sdk.NewInt64Coin(denoms.DenomNUSD, 1000),
...
func TestMsgServerRemoveMargin(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			vaultFunds: sdk.NewCoins(sdk.NewInt64Coin(denoms.DenomNUSD, 1000)),
			initialPosition: &types.Position{
				Pair:                            common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD),
				Size_:                           sdk.OneDec(),
				Margin:                          sdk.OneDec(),
			...
			marginToRemove: sdk.NewInt64Coin(denoms.DenomNUSD, 1000),
			expectedErr:    types.ErrFailedRemoveMarginCanCauseBadDebt,
		},
		{
			name:            "no initial position",
			vaultFunds:      sdk.NewCoins(sdk.NewInt64Coin(denoms.DenomNUSD, 0)),
			initialPosition: nil,
			marginToRemove:  sdk.NewInt64Coin(denoms.DenomNUSD, 1000),
			expectedErr:     collections.ErrNotFound,
		},
		{
			...
			vaultFunds: sdk.NewCoins(sdk.NewInt64Coin(denoms.DenomNUSD, 999)),
			initialPosition: &types.Position{
				Pair:                            common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD),
				Size_:                           sdk.OneDec(),
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil {
				...
				app.OracleKeeper.SetPrice(ctx, common.AssetRegistry.Pair(denoms.DenomBTC, denoms.DenomNUSD), sdk.OneDec())
...
func TestMsgServerDonateToEcosystemFund(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			donation:     sdk.NewInt64Coin(denoms.DenomNUSD, 100),
			expectedErr:  fmt.Errorf("insufficient funds"),
		},
		{
			name:         "success",
			sender:       testutil.AccAddress(),
			initialFunds: sdk.NewCoins(sdk.NewInt64Coin(denoms.DenomNUSD, 1e6)),
			donation:     sdk.NewInt64Coin(denoms.DenomNUSD, 100),
			expectedErr:  nil,
		},
	}
