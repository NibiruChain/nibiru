...
// 		aliceSide := v2types.Direction_LONG
...
// 		_, err = nibiruApp.PerpKeeperV2.OpenPosition(
...
// 		bobSide := v2types.Direction_LONG
...
// 		posResp, err := nibiruApp.PerpKeeperV2.ClosePosition(ctx, pair, alice)
// 		require.NoError(t, err)
// 		require.True(t, posResp.BadDebt.IsZero())
// 		require.True(t, !posResp.FundingPayment.IsZero() && posResp.FundingPayment.IsPositive())

// 		position, err := nibiruApp.PerpKeeperV2.Positions.Get(ctx, collections.Join(pair, alice))
...
// 		_, err = nibiruApp.PerpKeeperV2.OpenPosition(ctx, pair, aliceSide, alice, aliceQuote, aliceLeverage, aliceBaseLimit)
// 		require.NoError(t, err)
// 	})
// }
