...
func TestExecuteFullLiquidation(t *testing.T) {
	...
	testCases := map[string]test{
		"happy path - Buy": {
			...
			// startingBalance = 1* common.MICRO
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			assert.NoError(t, vpoolKeeper.CreatePool(
				ctx,
				tokenPair,
				/* quoteAssetReserves */ sdk.NewDec(10*common.MICRO),
				/* baseAssetReserves */ sdk.NewDec(5*common.MICRO),
			...
			startingModuleFunds := sdk.NewCoins(sdk.NewInt64Coin(
				tokenPair.QuoteDenom(), 1*common.MICRO))
	...
	for _, testCase := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NoError(t, vpoolKeeper.CreatePool(
				ctx,
				tokenPair,
				/* quoteAssetReserves */ sdk.NewDec(10_000*common.MICRO*common.MICRO),
				/* baseAssetReserves */ sdk.NewDec(5_000*common.MICRO*common.MICRO),
				vpooltypes.VpoolConfig{
					TradeLimitRatio:        sdk.MustNewDecFromStr("0.9"),
					FluctuationLimitRatio:  sdk.OneDec(),
					MaxOracleSpreadRatio:   sdk.MustNewDecFromStr("0.1"),
