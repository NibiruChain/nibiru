	...
	tests := []struct {
	...
	}{
		{
			...
			given: func(ctx sdk.Context, mocks mockedDependencies, perpKeeper Keeper) {
				...
				perpKeeper.PairMetadata.Insert(ctx, common.PairBTCStable,
					types.PairMetadata{
						Pair: common.PairBTCStable,
...
func TestClosePositionEntirely(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			perpKeeper.Positions.Insert(
				ctx,
				keys.Join(tc.initialPosition.Pair, keys.String(tc.initialPosition.TraderAddress)),
				tc.initialPosition,
			)
			...
			t.Log("set up pair metadata and last cumulative premium fraction")
			perpKeeper.PairMetadata.Insert(ctx, tc.pairMetadata.Pair, tc.pairMetadata)
...
func TestCloseAndOpenReversePosition(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			perpKeeper.Positions.Insert(ctx, keys.Join(currentPosition.Pair, keys.String(currentPosition.TraderAddress)), currentPosition)
...
func TestTransferFee(t *testing.T) {
	setup := func() (
		...
		perpKeeper.PairMetadata.Insert(ctx, metadata.Pair, *metadata)
...
func TestClosePosition(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Log("set position")
			perpKeeper.Positions.Insert(ctx, keys.Join(tc.initialPosition.Pair, keys.String(tc.initialPosition.TraderAddress)), tc.initialPosition)
