...
// TODO(mercilex): test
func (k Keeper) SettlePosition(ctx sdk.Context, pair common.TokenPair, trader string) error {
	...
	// check if pos size != 0
	if pos.Size_.IsZero() {
		return types.ErrPositionSizeZero
	}
	// clear pos
	...
	case true:
		settledValue = pos.Margin.ToDec()
	default:
		// openPrice = positionOpenNotional / abs(positionSize)
		openPrice := pos.OpenNotional.Quo(pos.Size_.Abs()).ToDec()
		// returnedFund := positionSize * (settlementPrice - openPrice) + positionMargin
		returnedFund := pos.Size_.ToDec().Mul(settlementPrice.Sub(openPrice)).Add(pos.Margin.ToDec())
