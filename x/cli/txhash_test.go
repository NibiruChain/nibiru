...
func TestClassifyTxHash(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "real CometBFT hash",
			input:     realCometTxHash,
			wantType:  CometBFTTxHash{},
			canonical: realCometTxHash,
		},
		{
			name:      "lowercase CometBFT hash",
			input:     strings.ToLower(realCometTxHash),
			wantType:  CometBFTTxHash{},
			canonical: realCometTxHash,
		},
		{
			name:      "another real CometBFT hash",
			input:     realCometTxHash2,
			wantType:  CometBFTTxHash{},
			canonical: realCometTxHash2,
		},
		{
			name:      "real EVM hash",
			input:     realEVMHash,
			wantType:  EVMTxHash{},
			canonical: realEVMHash,
		},
		{
			name:      "another real EVM hash",
			input:     realEVMHash2,
			wantType:  EVMTxHash{},
			canonical: realEVMHash2,
		},
		{
			name:      "uppercase EVM prefix and body",
			input:     "0X" + strings.ToUpper(strings.TrimPrefix(realEVMHash, "0x")),
			wantType:  EVMTxHash{},
			canonical: realEVMHash,
		},
		{
			name:      "unprefixed EVM-shaped input follows CometBFT grammar",
			input:     strings.TrimPrefix(realEVMHash, "0x"),
			wantType:  CometBFTTxHash{},
			canonical: strings.ToUpper(strings.TrimPrefix(realEVMHash, "0x")),
		},
		{
			name:      "zero CometBFT hash",
			input:     strings.Repeat("0", txHashHexLength),
			wantType:  CometBFTTxHash{},
			canonical: strings.Repeat("0", txHashHexLength),
		},
	}
...
func TestClassifyTxHashUsesGethTransactionHash(t *testing.T) {
	...
	require.Equal(t, input, evmHash.Canonical())
}
...
func TestClassifyTxHashInvalid(t *testing.T) {
	tests := []struct {
		...
		{name: "empty", input: ""},
		{name: "leading whitespace", input: " " + realCometTxHash},
		{name: "trailing whitespace", input: realCometTxHash + "\n"},
		{name: "too short", input: realCometTxHash[:txHashHexLength-1]},
		{name: "too long", input: realCometTxHash + "0"},
		{name: "invalid CometBFT character", input: realCometTxHash[:10] + "g" + realCometTxHash[11:]},
		{name: "invalid EVM character", input: realEVMHash[:10] + "g" + realEVMHash[11:]},
		{name: "EVM prefix without hash", input: "0x"},
		{name: "unsupported prefix", input: "0y" + realCometTxHash},
	}
...
func TestCometBFTTxHashBytesAreCopied(t *testing.T) {
	hash, err := ClassifyTxHash(realCometTxHash)
	require.NoError(t, err)
	cometHash := hash.(CometBFTTxHash)

	bytes := cometHash.Bytes()
	bytes[0] = 0

	require.Equal(t, realCometTxHash, cometHash.Canonical())
	require.NotEqual(t, bytes, cometHash.Bytes())
}

func TestEVMTxHashReturnsHashByValue(t *testing.T) {
	hash, err := ClassifyTxHash(realEVMHash)
	require.NoError(t, err)
	evmHash := hash.(EVMTxHash)

	gethHash := evmHash.Hash()
	gethHash[0] = 0

	require.Equal(t, realEVMHash, evmHash.Canonical())
	require.NotEqual(t, gethHash, evmHash.Hash())
}
