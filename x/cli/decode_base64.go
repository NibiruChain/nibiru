...
func YieldStargateMsgs(jsonBz []byte) (sgMsgs []wvm.StargateMsg, err error) {
	var data any
...
func parseStargateMsgs(jsonData any, msgs *[]wvm.StargateMsg) {
	switch v := jsonData.(type) {
	case map[string]any:
		if typeURL, ok := v["type_url"].(string); ok {
			if value, ok := v["value"].(string); ok {
				*msgs = append(*msgs, wvm.StargateMsg{
					TypeURL: typeURL,
					Value:   []byte(value),
				})
			}
