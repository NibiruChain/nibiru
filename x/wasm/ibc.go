...
func (i IBCHandler) OnChanOpenInit(
	...
	if err := i.keeper.ClaimCapability(ctx, chanCap, host.ChannelCapabilityPath(portID, channelID)); err != nil {
		return "", sdkioerrors.Wrap(err, "claim capability")
	}
...
func (i IBCHandler) OnChanOpenAck(
	...
	if !ok {
		return sdkioerrors.Wrapf(channeltypes.ErrChannelNotFound, "port ID (%s) channel ID (%s)", portID, channelID)
	}
...
func (i IBCHandler) OnRecvPacket(
	...
	if err != nil {
		...
		panic(sdkioerrors.Wrapf(err, "contract port id"))
	}
...
func (i IBCHandler) OnAcknowledgementPacket(
	...
	if err != nil {
		return sdkioerrors.Wrap(err, "on ack")
	}
	return nil
}
...
func ValidateChannelParams(channelID string) error {
	...
	if channelSequence > math.MaxUint32 {
		return sdkioerrors.Wrapf(types.ErrMaxIBCChannels, "channel sequence %d is greater than max allowed transfer channels %d", channelSequence, math.MaxUint32)
	}
	return nil
}
