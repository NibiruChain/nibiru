...
// DefaultWasmAppFactory instantiates and sets up the default Nibiru app.
func DefaultWasmAppFactory(t *testing.T, valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, chainID string, opts []wasmkeeper.Option, balances ...banktypes.Balance) ChainApp {
	...
	for _, balance := range balances {
		...
		require.NoError(t, err)
		require.NoError(t, testapp.FundAccount(nibiru.BankKeeper, ctx, addr, balance.Coins))
	}
	return nibiru
}
...
func (chain *TestChain) SendMsgs(msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	_, r, gotErr := signAndDeliverWithoutCommit(
		chain.t,
		chain.TxConfig,
...
func (chain *TestChain) ConstructUpdateTMClientHeaderWithTrustedHeight(counterparty *TestChain, clientID string, trustedHeight clienttypes.Height) (*ibctm.Header, error) {
	...
	if trustedHeight == counterparty.LastHeader.GetHeight() {
		...
		if !ok {
			return nil, sdkioerrors.Wrapf(ibctm.ErrInvalidHeaderHeight, "could not retrieve trusted validators at trustedHeight: %d", trustedHeight)
		}
	}
...
func (chain *TestChain) CreateTMClientHeader(chainID string, blockHeight int64, trustedHeight clienttypes.Height, timestamp time.Time, tmValSet, nextVals, tmTrustedVals *cmttypes.ValidatorSet, signers map[string]cmttypes.PrivValidator) *ibctm.Header {
	...
	voteSet := cmttypes.NewVoteSet(chainID, blockHeight, 1, tmproto.PrecommitType, tmValSet)
	...
	signerArr := make([]cmttypes.PrivValidator, len(tmValSet.Validators)) //nolint:staticcheck
