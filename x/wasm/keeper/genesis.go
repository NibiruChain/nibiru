...
func InitGenesis(ctx sdk.Context, keeper *Keeper, data types.GenesisState) ([]abci.ValidatorUpdate, error) {
	...
	if err != nil {
		return nil, sdkioerrors.Wrapf(err, "set params")
	}
	...
	for i, code := range data.Codes {
		...
		if err != nil {
			return nil, sdkioerrors.Wrapf(err, "code %d with id: %d", i, code.CodeID)
		}
		...
		if code.Pinned {
			if err := contractKeeper.PinCode(ctx, code.CodeID); err != nil {
				return nil, sdkioerrors.Wrapf(err, "contract number %d", i)
			}
		}
	}
	...
	for i, contract := range data.Contracts {
		...
		if err != nil {
			return nil, sdkioerrors.Wrapf(err, "address in contract number %d", i)
		}
	...
	for i, seq := range data.Sequences {
		...
		if err != nil {
			return nil, sdkioerrors.Wrapf(err, "sequence number %d", i)
		}
	}
	...
	if seqVal <= maxCodeID {
		return nil, sdkioerrors.Wrapf(types.ErrInvalid, "seq %s with value: %d must be greater than: %d ", string(types.KeySequenceCodeID), seqVal, maxCodeID)
	}
	...
	if keeper.HasContractInfo(ctx, addr) {
		return nil, sdkioerrors.Wrapf(types.ErrInvalid, "value: %d for seq %s was used already", seqVal, string(types.KeySequenceInstanceID))
	}
	return nil, nil
}
