...
type testData struct {
	...
	encConf          nibiruapp.EncodingConfig
}
...
func newTestFaucet(tb testing.TB, accountKeeper authkeeper.AccountKeeper, bankKeeper bankkeeper.Keeper) *testFaucet {
	tb.Helper()
	return &testFaucet{
		t:             tb,
...
func (f *testFaucet) NewFundedRandomAccount(ctx sdk.Context, coins ...sdk.Coin) sdk.AccAddress {
	f.t.Helper()
	addr := keyPubAddr()
	f.accountKeeper.SetAccount(ctx, authtypes.NewBaseAccountWithAddress(addr))
	f.Fund(ctx, addr, coins...)
	return addr
}

func (f *testFaucet) Fund(ctx sdk.Context, addr sdk.AccAddress, coins ...sdk.Coin) {
	f.t.Helper()
	require.NoError(f.t, testapp.FundAccount(f.bankKeeper, ctx, addr, sdk.NewCoins(coins...)))
}

func setupTest(t *testing.T) testData {
	t.Helper()
	nibiru, ctx := testapp.NewNibiruTestAppAndContext()
	encConf := nibiruapp.MakeEncodingConfig()
	...
	data := testData{
		module:           wasm.NewAppModule(encConf.Codec, &nibiru.WasmKeeper, nibiru.StakingKeeper, nibiru.AccountKeeper, nibiru.BankKeeper, nil, newMockSubspace(types.DefaultParams())),
		ctx:              ctx,
		acctKeeper:       nibiru.AccountKeeper,
		keeper:           nibiru.WasmKeeper,
		bankKeeper:       nibiru.BankKeeper,
		stakingKeeper:    nibiru.StakingKeeper,
		faucet:           newTestFaucet(t, nibiru.AccountKeeper, nibiru.BankKeeper),
