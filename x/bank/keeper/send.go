...
type BaseSendKeeper struct {
	...
	weiBlockDeltasCommitted collections.Item[sdkmath.Int]
}

func NewBaseSendKeeper(
	cdc codec.BinaryCodec,
	storeKey, transientStoreKey storetypes.StoreKey,
	ak types.AccountKeeper,
	blockedAddrs map[string]bool,
	authority string,
) BaseSendKeeper {
	...
	return BaseSendKeeper{
		...
		weiBlockDelta: collections.NewItemTransient(
			...
			collections.SignedIntValueEncoder,
		),
		// TODO: UD-DEBUG: cleanup and remove weiBlockDeltasCommitted
		weiBlockDeltasCommitted: collections.NewItem(
			storeKey,
			NAMESPACE_WEI_COMMITTED_DELTA,
			collections.SignedIntValueEncoder,
		),
	}
}
...
func (k BaseSendKeeper) InputOutputCoins(ctx sdk.Context, inputs []types.Input, outputs []types.Output) error {
	...
	for _, in := range inputs {
		...
		hasWeiChange, _ := in.Coins.Find(DENOM_UNIBI)
		if hasWeiChange {
			weiChangeAddrs = append(weiChangeAddrs, in.Address)
		}
	...
	for _, out := range outputs {
		...
		hasWeiChange, _ := out.Coins.Find(DENOM_UNIBI)
		if hasWeiChange {
			weiChangeAddrs = append(weiChangeAddrs, out.Address)
		}
	...
	if len(weiChangeAddrs) > 0 {
		ctx.EventManager().EmitEvent(
			types.NewEventWeiChange(
				types.WeiChangeReason_InputOutputCoins,
				weiChangeAddrs...,
			),
		)
	}
...
func (k BaseSendKeeper) SendCoins(ctx sdk.Context, fromAddr sdk.AccAddress, toAddr sdk.AccAddress, amt sdk.Coins) error {
	...
	ctx.EventManager().EmitEvents(eventsForSendCoins(fromAddr, toAddr, amt))
