...
func NewBaseKeeper(
	...
	return BaseKeeper{
		BaseSendKeeper:    NewBaseSendKeeper(cdc, storeKey, transientStoreKey, ak, blockedAddrs, authority),
		ak:                ak,
		cdc:               cdc,
		storeKey:          storeKey,
...
func (k BaseKeeper) DelegateCoins(ctx sdk.Context, delegatorAddr, moduleAccAddr sdk.AccAddress, amt sdk.Coins) error {
	...
	for _, coin := range amt {
		...
		if hasWeiChange || coin.GetDenom() == DENOM_UNIBI {
			hasWeiChange = true
		}
	}
	...
	ctx.EventManager().EmitEvent(
		types.NewEventWeiChange(
			types.WeiChangeReason_DelegateCoins,
			moduleAcc.String(), delegatorAddr.String(),
		),
	)
...
func (k BaseKeeper) UndelegateCoins(ctx sdk.Context, moduleAccAddr, delegatorAddr sdk.AccAddress, amt sdk.Coins) error {
	...
	if hasWeiChange, _ := amt.Find(DENOM_UNIBI); hasWeiChange {
		ctx.EventManager().EmitEvent(
			types.NewEventWeiChange(
				types.WeiChangeReason_UndelegateCoins,
				moduleAcc.String(), delegatorAddr.String(),
			),
		)
	}
...
func (k BaseKeeper) MintCoins(ctx sdk.Context, moduleName string, amounts sdk.Coins) error {
	...
	if hasWeiChange, _ := amounts.Find(DENOM_UNIBI); hasWeiChange {
		ctx.EventManager().EmitEvent(
			types.NewEventWeiChange(
				types.WeiChangeReason_MintCoins, acc.GetAddress().String()),
		)
	}
...
func (k BaseKeeper) BurnCoins(ctx sdk.Context, moduleName string, amounts sdk.Coins) error {
	...
	if hasWeiChange, _ := amounts.Find(DENOM_UNIBI); hasWeiChange {
		ctx.EventManager().EmitEvent(
			types.NewEventWeiChange(
				types.WeiChangeReason_BurnCoins, acc.GetAddress().String()),
		)
	}
