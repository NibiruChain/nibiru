...
func (s *MsgsSuite) TestMsgEthereumTx_Constructor() {
	...
	s.Require().Equal(msg.Route(), types.RouterKey)
	s.Require().Equal(msg.Type(), types.TypeMsgEthereumTx)
...
func (s *MsgsSuite) TestMsgEthereumTx_BuildTx() {
	...
	for _, tc := range testCases {
		...
		tx, err := tc.msg.BuildTx(s.clientCtx.TxConfig.NewTxBuilder(), types.DefaultEVMDenom)
		if tc.expError {
			s.Require().Error(err)
		} else {
			...
			s.Require().Equal(sdk.NewCoins(sdk.NewCoin(types.DefaultEVMDenom, sdkmath.NewInt(100000))), tx.GetFee())
		}
	}
}
...
func (s *MsgsSuite) TestMsgEthereumTx_Sign() {
	testCases := []struct {
	...
	}{
		{
			...
			func(tx *types.MsgEthereumTx) { tx.From = s.from.Hex() },
			true,
		},
		{
			"pass - EIP155 signer",
		...
		},
		{
			...
			gethcore.NewEIP2930Signer(s.chainID),
			func(tx *types.MsgEthereumTx) { tx.From = s.to.Hex() },
			false,
		},
	}
...
func encodeDecodeBinary(tx *gethcore.Transaction) (*types.MsgEthereumTx, error) {
...
func (s *MsgsSuite) TestUnwrapEthererumMsg() {
	_, err := types.UnwrapEthereumMsg(nil, common.Hash{})
	s.NotNil(err)
	...
	_, err = types.UnwrapEthereumMsg(&tx, common.Hash{})
	s.NotNil(err)

	evmTxParams := &types.EvmTxArgs{
		ChainID:  big.NewInt(1),
		Nonce:    0,
		To:       &common.Address{},
		Amount:   big.NewInt(0),
	...
	msg := types.NewTx(evmTxParams)
	err = builder.SetMsgs(msg)
	s.Nil(err)

	tx = builder.GetTx().(sdk.Tx)
	unwrappedMsg, err := types.UnwrapEthereumMsg(&tx, msg.AsTransaction().Hash())
	s.Nil(err)
...
func (s *MsgsSuite) TestTransactionLogsEncodeDecode() {
	...
	txLogsEncoded, encodeErr := types.EncodeTransactionLogs(&txLogs)
	s.Nil(encodeErr)

	txLogsEncodedDecoded, decodeErr := types.DecodeTransactionLogs(txLogsEncoded)
	s.Nil(decodeErr)
