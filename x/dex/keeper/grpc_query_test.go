		...
		},
		{
			...
			newPools: []types.Pool{
				mock.DexPool(
					...
					/*assets=*/ sdk.NewCoins(
						sdk.NewInt64Coin("unibi", 100),
						sdk.NewInt64Coin(denoms.DenomUSDC, 100),
					),
					/*shares=*/ 100,
				),
				mock.DexPool(
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, simapp.FundAccount(
				...
				sender,
				sdk.NewCoins(
					sdk.NewInt64Coin(denoms.DenomNIBI, 1e18),
					sdk.NewInt64Coin(denoms.DenomNUSD, 1e18),
					sdk.NewInt64Coin(denoms.DenomUSDC, 1e18),
				),
			))
...
func TestQueryEstimateSwapExactAmountIn(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			tokenIn:          sdk.NewInt64Coin(denoms.DenomNUSD, 100),
			tokenOutDenom:    "unibi",
		...
		},
		{
			...
			existingPool: mock.DexPool(
				...
				/*assets=*/ sdk.NewCoins(
					...
					sdk.NewInt64Coin(denoms.DenomNUSD, 4684496849),
				),
				/*shares=*/ 100,
			),
			...
			expectedTokenOut: sdk.NewInt64Coin(denoms.DenomNUSD, 77588330),
		},
	}
...
func TestQueryEstimateSwapExactAmountOut(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedTokenIn: sdk.NewInt64Coin(denoms.DenomNUSD, 101),
		},
		{
			name: "complex swap",
			existingPool: mock.DexPool(
			...
			tokenOut:     sdk.NewInt64Coin(denoms.DenomNUSD, 77588330),
			tokenInDenom: "unibi",
			// https://www.wolframalpha.com/input?i=4684496849+-+%2834844867+*+4684496849+%2F+%2834844867%2B586848%29+%29
