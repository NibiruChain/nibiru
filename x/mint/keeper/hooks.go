...
func (h Hooks) AfterEpochEnd(ctx sdk.Context, epochIdentifier string, epochNumber uint64) {
	...
	epochMintProvision := mint.CalculateEpochMintProvision(
		params,
		period,
	)
	...
	defer func() {
		...
		if mintedCoin.Amount.IsInt64() {
			telemetry.IncrCounterWithLabels(
				[]string{mint.ModuleName, "allocate", "total"},
		...
		if stakingAmt.IsInt64() {
			telemetry.IncrCounterWithLabels(
				[]string{mint.ModuleName, "allocate", "staking", "total"},
		...
		if strategicAmt.IsInt64() {
			telemetry.IncrCounterWithLabels(
				[]string{mint.ModuleName, "allocate", "strategic", "total"},
		...
		if cpAmt.IsInt64() {
			telemetry.IncrCounterWithLabels(
				[]string{mint.ModuleName, "allocate", "community_pool", "total"},
				float32(cpAmt.Int64()),
				[]metrics.Label{telemetry.NewLabel("denom", mintedCoin.Denom)},
			)
		}
	...
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			mint.EventTypeInflation,
			sdk.NewAttribute(mint.AttributeEpochNumber, fmt.Sprintf("%d", epochNumber-numSkippedEpochs)),
			sdk.NewAttribute(mint.AttributeKeyEpochProvisions, epochMintProvision.String()),
