...
func NewDeductFeeDecorator(gtk keeper.Keeper, ek *evmkeeper.Keeper, ak types.AccountKeeper, bk authtypes.BankKeeper, fk types.FeegrantKeeper, ok oraclekeeper.Keeper) DeductFeeDecorator {
...
func (dfd DeductFeeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	if len(fee) != 1 {
		return ctx, sdkioerrors.Wrapf(sdkerrors.ErrInvalidRequest, "only one fee token is supported, got %d", len(fee))
	}
...
func DeductFees(accountkeeper types.AccountKeeper, ek *evmkeeper.Keeper, gtk keeper.Keeper, bankKeeper authtypes.BankKeeper, ok oraclekeeper.Keeper, ctx sdk.Context, acc authtypes.AccountI, fees sdk.Coins) error {
	...
	if fees[0].Denom == appconst.BondDenom {
		...
		feeToken, err := gtk.GetFeeToken(ctx, strings.TrimPrefix(fees[0].Denom, "erc20/"))
		if err != nil {
		...
		err = ek.Erc20Transfer(ctx, gethcommon.HexToAddress(feeToken.Address), sender, feeCollector, evm.NativeToWei(amount))
		if err != nil {
			return err
		}
		...
		err = ProcessFeeToken(ctx, ek, accountkeeper, gtk, feeToken, feeCollector)
		if err != nil {
			return err
		}
	}
...
func ProcessFeeToken(ctx sdk.Context, ek *evmkeeper.Keeper, accountkeeper types.AccountKeeper, gastokenkeeper keeper.Keeper, feeToken types.FeeToken, feeCollector gethcommon.Address) error {
	...
	if feeToken.TokenType == types.FeeTokenType_FEE_TOKEN_TYPE_CONVERTIBLE {
		...
		if err := SwapFeeToken(ctx, ek, accountkeeper, gastokenkeeper, feeToken, feeCollector); err != nil {
			return sdkioerrors.Wrapf(err, "failed to swap fee token %s", feeToken.Address)
		}

		// Then withdraw
		baseToken, err := gastokenkeeper.GetBaseToken(ctx, "WNIBI")
		if err != nil {
...
func SwapFeeToken(ctx sdk.Context, ek *evmkeeper.Keeper, ak types.AccountKeeper, gtk keeper.Keeper, feeToken types.FeeToken, feeCollector gethcommon.Address) error {
	...
	amountIn := out
	baseToken, err := gtk.GetBaseToken(ctx, "WNIBI")
