...
func TestSwapOutput_Errors(t *testing.T) {
	tests := []struct {
		...
		baseAmount sdktypes.Int
		quoteLimit sdktypes.Int
		error      error
	}{
		{
			"pair not supported",
		...
		{
			"base input bigger than reserve ratio",
			UsdmPair,
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := keeper.CreatePool(
				ctx,
				UsdmPair,
				sdktypes.MustNewDecFromStr("0.9"), // 0.9 trade limit ratio
...
func TestSwapOutput_HappyPath(t *testing.T) {
	tests := []struct {
		name                 string
		direction            ammtypes.Direction
		baseAmount           sdktypes.Int
