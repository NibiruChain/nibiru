...
func TestOracleTallyTiming(t *testing.T) {
	...
	for i := range keeper.Addrs[:4] {
		keeper.MakeAggregatePrevoteAndVote(t, input, h, 0, types.ExchangeRateTuples{
			{Pair: asset.Registry.Pair(denoms.BTC, denoms.UUSD), ExchangeRate: sdkmath.LegacyOneDec()},
		}, i)
	}
	...
	_, err = input.OracleKeeper.ExchangeRates.Get(input.Ctx, asset.Registry.Pair(denoms.BTC, denoms.UUSD))
	require.Error(t, err)
...
func TestOraclePriceExpiration(t *testing.T) {
	input, h := keeper.Setup(t)
	pair1 := asset.Registry.Pair(denoms.BTC, denoms.UUSD)
	pair2 := asset.Registry.Pair(denoms.ETH, denoms.UUSD)
