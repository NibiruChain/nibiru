...
func TestKeeper_RewardsDistributionMultiVotePeriods(t *testing.T) {
	...
	keeper.AllocateRewards(t, input, common.AssetRegistry.Pair(common.DenomNIBI, common.DenomNUSD), sdk.NewCoins(rewards), periods)
	...
	require.True(t, input.OracleKeeper.GatherRewardsForVotePeriod(input.Ctx, common.AssetRegistry.Pair(common.DenomNIBI, common.DenomNUSD)).IsZero())

	// assert that there are no rewards instances
	require.Empty(t, input.OracleKeeper.PairRewards.Indexes.RewardsByPair.ExactMatch(input.Ctx, common.AssetRegistry.Pair(common.DenomNIBI, common.DenomNUSD)).PrimaryKeys())
}
