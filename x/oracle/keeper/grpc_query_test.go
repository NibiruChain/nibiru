...
func TestQueryExchangeRateTwap(t *testing.T) {
	...
	_, err := querier.ExchangeRateTwap(input.Ctx, &types.QueryExchangeRateRequest{Pair: asset.Registry.Pair(denoms.ETH, denoms.NUSD)})
	require.Error(t, err)

	res, err := querier.ExchangeRateTwap(input.Ctx, &types.QueryExchangeRateRequest{Pair: asset.Registry.Pair(denoms.BTC, denoms.NUSD)})
	require.NoError(t, err)
