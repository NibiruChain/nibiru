...
func TestInvalidVotesSlashing(t *testing.T) {
	...
	input.OracleKeeper.WhitelistedPairs.Insert(input.Ctx, asset.Registry.Pair(denoms.ATOM, denoms.UUSD))
	...
	for i := uint64(0); i < uint64(sdkmath.LegacyOneDec().Sub(minValidPerWindow).MulInt64(votePeriodsPerWindow).TruncateInt64()); i++ {
		...
		MakeAggregatePrevoteAndVote(t, input, h, 0, types.ExchangeRateTuples{
			{Pair: asset.Registry.Pair(denoms.ATOM, denoms.UUSD), ExchangeRate: testExchangeRate.Add(sdkmath.LegacyNewDec(100000000000000))},
		}, 1)
...
func TestNotPassedBallotSlashing(t *testing.T) {
	...
	// Account 1, govstable
	MakeAggregatePrevoteAndVote(t, input, h, 0, types.ExchangeRateTuples{{Pair: asset.Registry.Pair(denoms.ATOM, denoms.UUSD), ExchangeRate: testExchangeRate}}, 0)
...
func TestAbstainSlashing(t *testing.T) {
	...
	for i := uint64(0); i <= uint64(sdkmath.LegacyOneDec().Sub(minValidPerWindow).MulInt64(votePeriodsPerWindow).TruncateInt64()); i++ {
		...
		MakeAggregatePrevoteAndVote(t, input, h, 0, types.ExchangeRateTuples{{Pair: asset.Registry.Pair(denoms.ATOM, denoms.UUSD), ExchangeRate: sdkmath.LegacyOneDec().Neg()}}, 1)

		// Account 3, ATOM/USD
		MakeAggregatePrevoteAndVote(t, input, h, 0, types.ExchangeRateTuples{{Pair: asset.Registry.Pair(denoms.ATOM, denoms.UUSD), ExchangeRate: testExchangeRate}}, 2)
