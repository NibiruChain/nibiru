...
func TestInvalidVotesSlashing(t *testing.T) {
	...
	for i := uint64(0); i < uint64(types.OneDec().Sub(minValidPerWindow).MulInt64(votePeriodsPerWindow).TruncateInt64()); i++ {
		...
		makeAggregatePrevoteAndVote(t, input, h, 0, types3.ExchangeRateTuples{{Pair: common.Pair_NIBI_NUSD.String(), ExchangeRate: randomExchangeRate.Add(types.NewDec(100000000000000))}}, 1)
...
func TestWhitelistSlashing(t *testing.T) {
	...
	for i := uint64(0); i < uint64(types.OneDec().Sub(minValidPerWindow).MulInt64(votePeriodsPerWindow).TruncateInt64()); i++ {
		...
		// Account 2, govstable
		makeAggregatePrevoteAndVote(t, input, h, 0, types3.ExchangeRateTuples{{Pair: common.Pair_BTC_NUSD.String(), ExchangeRate: randomExchangeRate}}, 1)
		// Account 3, govstable
		makeAggregatePrevoteAndVote(t, input, h, 0, types3.ExchangeRateTuples{{Pair: common.Pair_BTC_NUSD.String(), ExchangeRate: randomExchangeRate}}, 2)
...
func TestAbstainSlashing(t *testing.T) {
	...
	for i := uint64(0); i <= uint64(types.OneDec().Sub(minValidPerWindow).MulInt64(votePeriodsPerWindow).TruncateInt64()); i++ {
		...
		makeAggregatePrevoteAndVote(t, input, h, 0, types3.ExchangeRateTuples{{Pair: common.Pair_NIBI_NUSD.String(), ExchangeRate: types.ZeroDec()}}, 1)
