...
func TestMsgServer_FeederDelegation(t *testing.T) {
	...
	salt := "1"
	hash := types.GetAggregateVoteHash(salt, randomExchangeRate.String()+common.MicroCollDenom, ValAddrs[0])
	...
	voteMsg := types.NewMsgAggregateExchangeRateVote(salt, randomExchangeRate.String()+common.MicroCollDenom, Addrs[0], ValAddrs[0])
	...
	voteMsg = types.NewMsgAggregateExchangeRateVote(salt, randomExchangeRate.String()+common.MicroCollDenom, Addrs[1], ValAddrs[0])
	_, err = msgServer.AggregateExchangeRateVote(sdk.WrapSDKContext(input.Ctx.WithBlockHeight(1)), voteMsg)
	require.Error(t, err)
	...
	voteMsg = types.NewMsgAggregateExchangeRateVote(salt, randomExchangeRate.String()+common.MicroCollDenom, Addrs[2], ValAddrs[0])
	_, err = msgServer.AggregateExchangeRateVote(sdk.WrapSDKContext(input.Ctx.WithBlockHeight(1)), voteMsg)
	require.Error(t, err)
...
func TestMsgServer_AggregatePrevoteVote(t *testing.T) {
	...
	salt := "1"
	exchangeRatesStr := fmt.Sprintf("1000.23%s,0.29%s,0.27%s", common.DenomStable, core.MicroUSDDenom, common.MicroCollDenom)
