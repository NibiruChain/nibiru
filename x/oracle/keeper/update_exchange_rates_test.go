...
func TestOracleExchangeRate(t *testing.T) {
	...
	ethNusdRewards := sdk.NewInt64Coin("ETHREWARD", 1*common.MICRO)
	nibiNusdRewards := sdk.NewInt64Coin("NIBIREWARD", 1*common.MICRO)
	...
	// total reward pool for the current vote period is 1* common.MICRO for eth:nusd and 1* common.MICRO for nibi:nusd
