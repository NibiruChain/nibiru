...
func TestSnapshotUpdates(t *testing.T) {
	...
	snapshot, err := vpoolKeeper.ReserveSnapshots.Get(ctx, collections.Join(common.AssetRegistry.Pair(denoms.BTC, denoms.NUSD), time.UnixMilli(expectedSnapshot.TimestampMs)))
	...
	snapshot, err = vpoolKeeper.ReserveSnapshots.Get(ctx,
		collections.Join(common.AssetRegistry.Pair(denoms.BTC, denoms.NUSD), time.UnixMilli(expectedSnapshot.TimestampMs)))
	require.NoError(t, err)
	assert.EqualValues(t, expectedSnapshot, snapshot)
