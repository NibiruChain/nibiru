...
func (s *TestSuite) TestStargateSerde() {
	testCases := []struct {
		...
		},
		{
			...
			wantBz: fmt.Sprintf("%v", []byte{10, 6, 115, 101, 110, 100, 101, 114, 18, 13, 10, 6, 97, 98, 99, 120, 121, 122, 18, 3, 49, 50, 51, 26, 7, 109, 105, 110, 116, 95, 116, 111}),
		},
	}

	for _, tc := range testCases {
		s.Run(tc.typeUrl, func() {
			pbMsg, _ := (tc.sdkMsg).(proto.Message)
