...
func NewMsgServerImpl(keeper Keeper) inflation.MsgServer {
...
func (ms msgServer) EditInflationParams(
	goCtx context.Context, msg *inflation.MsgEditInflationParams,
) (resp *inflation.MsgEditInflationParamsResponse, err error) {
	...
	resp = &inflation.MsgEditInflationParamsResponse{}
	return resp, err
}
...
func (ms msgServer) ToggleInflation(
	goCtx context.Context, msg *inflation.MsgToggleInflation,
) (resp *inflation.MsgToggleInflationResponse, err error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	// Stateless field validation was already performed in msg.ValidateBasic()
	// before the current scope is reached.
	sender, _ := sdk.AccAddressFromBech32(msg.Sender)
	err = ms.Sudo().ToggleInflation(ctx, msg.Enable, sender)
	resp = &inflation.MsgToggleInflationResponse{}
	return resp, err
}
