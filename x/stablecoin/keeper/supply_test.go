...
func TestKeeper_GetStableMarketCap(t *testing.T) {
	...
	err := k.BankKeeper.MintCoins(ctx, types.ModuleName, sdk.NewCoins(
		sdk.NewInt64Coin(denoms.NUSD, 1*common.MICRO),
	))
	...
	require.Equal(t, sdk.NewInt(1*common.MICRO), marketCap)
}
...
func TestKeeper_GetGovMarketCap(t *testing.T) {
	...
	poolAssets := []spottypes.PoolAsset{
		{
			Token:  sdk.NewInt64Coin(denoms.NIBI, 2*common.MICRO),
			Weight: sdk.NewInt(100),
		},
		{
			Token:  sdk.NewInt64Coin(denoms.NUSD, 1*common.MICRO),
			Weight: sdk.NewInt(100),
		},
	}
	...
	require.Equal(t, sdk.NewInt(2*common.MICRO), marketCap) // 1 * 10^6 * 2 (price of gov token)
}
