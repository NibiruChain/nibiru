...
func TestSetCollRatioUpdate(t *testing.T) {
	...
	executeTest := func(t *testing.T, testCase TestCase) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			oracleKeeper.SetPrice(ctx, common.AssetRegistry.Pair(denoms.DenomUSDC, denoms.DenomNUSD), tc.price)
			err := stablecoinKeeper.SetCollRatio(ctx, tc.inCollRatio)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, nibiruApp.BankKeeper.MintCoins(
				ctx, types.ModuleName, sdk.NewCoins(
					sdk.NewCoin(denoms.DenomUSDC, tc.protocolColl),
					sdk.NewCoin(denoms.DenomNUSD, tc.stableSupply),
				),
			))

			// Post prices to each specified market with the oracle.
			prices := map[common.AssetPair]sdk.Dec{
				common.AssetRegistry.Pair(denoms.DenomUSDC, denoms.DenomNUSD): tc.priceCollStable,
			}
...
func TestRecollateralizeCollAmtForTargetCollRatio(t *testing.T) {
	...
	for _, testCase := range expectedPasses {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Post the price
			pair := common.AssetRegistry.Pair(denoms.DenomUSDC, denoms.DenomNUSD)
...
func TestGovAmtFromFullRecollateralize(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			prices := map[common.AssetPair]sdk.Dec{
				common.AssetRegistry.Pair(denoms.DenomNIBI, denoms.DenomNUSD): tc.priceGovStable,
				common.AssetRegistry.Pair(denoms.DenomUSDC, denoms.DenomNUSD): tc.priceCollStable,
			}
			for _, pair := range tc.postedAssetPairs {
				nibiruApp.OracleKeeper.SetPrice(ctx, pair, prices[pair])
...
func TestRecollateralize(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			accFunds: sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomUSDC, 1_000*common.Precision),
			),

			expectedNeededUSD: sdk.NewDec(100_000),
			msg: types.MsgRecollateralize{
				Creator: testutil.AccAddress().String(),
				Coll:    sdk.NewCoin(denoms.DenomUSDC, sdk.NewInt(100_000)),
			},
			response: &types.MsgRecollateralizeResponse{
				/*
				...
				*/
				Gov: sdk.NewCoin(denoms.DenomNIBI, sdk.NewInt(100_200)),
			},
			expectedPass: true,
		},
		{
			...
			msg: types.MsgRecollateralize{
				Creator: testutil.AccAddress().String(),
				Coll:    sdk.NewCoin(denoms.DenomUSDC, sdk.NewInt(50_000)),
			},
			response: &types.MsgRecollateralizeResponse{
				/*
					Gov.Amount = inCollUSD * (1 + bonusRate) / priceGovStable
					  ...
					  = 11021.98998 -> 11_021
				*/
				Gov: sdk.NewCoin(denoms.DenomNIBI, sdk.NewInt(11_021)),
			},
			expectedPass: true,
		},
		{
			...
			accFunds: sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomUSDC, 1*common.Precision),
			),

			// Since 'neededUSD' is
			msg: types.MsgRecollateralize{
				Creator: testutil.AccAddress().String(),
				Coll:    sdk.NewCoin(denoms.DenomUSDC, sdk.NewInt(100)),
			},
			expectedPass: false,
		...
		},
		{
			...
			accFunds: sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomUSDC, 99),
			),

			// Since 'neededUSD' is
			msg: types.MsgRecollateralize{
				Creator: testutil.AccAddress().String(),
				Coll:    sdk.NewCoin(denoms.DenomUSDC, sdk.NewInt(200)),
			},
			expectedPass: false,
		...
		},
		{
			...
			accFunds: sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomUSDC, 400),
			),

			msg: types.MsgRecollateralize{
				Creator: testutil.AccAddress().String(),
				Coll:    sdk.Coin{Denom: denoms.DenomUSDC, Amount: sdk.NewInt(-200)},
			},
			expectedPass: false,
			err: fmt.Errorf(
				"collateral input, -200%v, must be positive", denoms.DenomUSDC),
		},
		{
			...
			msg: types.MsgRecollateralize{
				Creator: testutil.AccAddress().String(),
				Coll:    sdk.NewInt64Coin(denoms.DenomUSDC, 400),
			},
	...
	for _, testCase := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, nibiruApp.BankKeeper.MintCoins(
				ctx, types.ModuleName, sdk.NewCoins(
					sdk.NewCoin(denoms.DenomUSDC, tc.scenario.protocolColl),
					sdk.NewCoin(denoms.DenomNUSD, tc.scenario.stableSupply),
				),
			))
			// Fund account
			...
			prices := map[common.AssetPair]sdk.Dec{
				common.AssetRegistry.Pair(denoms.DenomNIBI, denoms.DenomNUSD): tc.priceGovStable,
				common.AssetRegistry.Pair(denoms.DenomUSDC, denoms.DenomNUSD): tc.scenario.priceCollStable,
			}
			for _, pair := range tc.postedAssetPairs {
				nibiruApp.OracleKeeper.SetPrice(ctx, pair, prices[pair])
			}
		...
		},
		{
			...
			test: func() {
				...
				msg := &types.MsgRecollateralize{
					...
					Coll:    sdk.NewInt64Coin(denoms.DenomUSDC, 100),
				}
				_, err := nibiruApp.StablecoinKeeper.Recollateralize(goCtx, msg)
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedAccFundsAfter: sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomNIBI, 900_000), // accFunds - inGov.Amount
				sdk.NewInt64Coin(denoms.DenomUSDC, 100_000), // response.Coll
			),

			expectedNeededUSD: sdk.NewDec(-100_000),
			msg: types.MsgBuyback{
				Creator: testutil.AccAddress().String(),
				Gov:     sdk.NewCoin(denoms.DenomNIBI, sdk.NewInt(100_000)),
			},
			response: &types.MsgBuybackResponse{
				/*
		...
		},
		{
			...
			expectedAccFundsAfter: sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomNIBI, 953_000), // accFunds - inGov.Amount
				sdk.NewInt64Coin(denoms.DenomUSDC, 213_636), // response.Coll
			),

			expectedNeededUSD: sdk.MustNewDecFromStr("-234999.15"),
			msg: types.MsgBuyback{
				Creator: testutil.AccAddress().String(),
				Gov:     sdk.NewCoin(denoms.DenomNIBI, sdk.NewInt(50_000)),
			},
			response: &types.MsgBuybackResponse{
				/*
				...
				*/
				Coll: sdk.NewCoin(denoms.DenomUSDC, sdk.NewInt(213_636)),
			},
			expectedPass: true,
		},
		{
			...
			msg: types.MsgBuyback{
				Creator: testutil.AccAddress().String(),
				Gov:     sdk.NewCoin(denoms.DenomNIBI, sdk.NewInt(200_000)),
			},
			response: &types.MsgBuybackResponse{
				// Coll.Amount = inUSD *  / priceCollStable
				Coll: sdk.NewCoin(denoms.DenomUSDC, sdk.NewInt(100_000)),
		...
		},
		{
			...
			priceGovStable: sdk.OneDec(),
			accFunds: sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomNIBI, 1_000*common.Precision),
			),
		...
		},
		{
			...
			priceGovStable: sdk.OneDec(),
			accFunds: sdk.NewCoins(
				sdk.NewInt64Coin(denoms.DenomNIBI, 1),
			),
