...
func TestMsgMintStableResponse_HappyPath(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			supplyGov: sdk.NewCoin(common.GovDenom, sdk.NewInt(10)),
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, nibiruApp.StablecoinKeeper.GetSupplyNIBI(ctx), tc.supplyGov)
			require.Equal(t, nibiruApp.StablecoinKeeper.GetSupplyNUSD(ctx), tc.supplyUsdm)
...
func TestMsgBurnResponse_HappyPath(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			supplyGov:     sdk.NewCoin(common.GovDenom, sdk.NewInt(100_000-100)), // nibiru minus 0.5 of fees burned (the part that goes to EF)
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.supplyGov, nibiruApp.StablecoinKeeper.GetSupplyNIBI(ctx))
			require.Equal(t, tc.supplyUsdm, nibiruApp.StablecoinKeeper.GetSupplyNUSD(ctx))
