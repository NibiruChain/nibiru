...
// EmitMintGov emits an event when NIBI is minted.
func EmitMintGov(ctx sdk.Context, coin sdk.Coin) {
	const EventTypeMintGov = "mint_nibi"
	ctx.EventManager().EmitEvent(
		_mintOrBurnEvent(EventTypeMintGov, coin),
	)
}

// EmitBurnGov emits an event when NIBI is burned.
func EmitBurnGov(ctx sdk.Context, coin sdk.Coin) {
	const EventTypeBurnGov = "burn_nibi"
	ctx.EventManager().EmitEvent(
		_mintOrBurnEvent(EventTypeBurnGov, coin),
	)
}
