...
func TestPostPrice(t *testing.T) {
	...
	mp := types.Params{
		Pairs: []types.Pair{
			{PairID: "tstusd", BaseAsset: "tst", QuoteAsset: "usd", Oracles: authorizedOracles, Active: true},
		},
	}
	k.SetParams(ctx, mp)
	...
	tests := []struct {
		...
		{"invalid", authorizedOracles[0], "invalid", ctx.BlockTime().UTC().Add(time.Hour * 1), false, types.ErrInvalidPair},
	...
	for _, tt := range tests {
		t.Run(tt.giveMsg, func(t *testing.T) {
			...
			msg := types.NewMsgPostPrice(tt.giveOracle.String(), tt.givePairId, sdk.MustNewDecFromStr("0.5"), tt.giveExpiry)
