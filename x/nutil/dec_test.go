...
func TestSqrtBigInt(t *testing.T) {
	testCases := []struct {
		...
		{
			bigInt:     new(big.Int).Mul(big.NewInt(4_819_136_400), nutil.BigIntPow10(32)),
			sqrtBigInt: new(big.Int).Mul(big.NewInt(69_420), nutil.BigIntPow10(16)),
		},
		{
			bigInt:     new(big.Int).Mul(big.NewInt(9), nutil.BigIntPow10(100)),
			sqrtBigInt: new(big.Int).Mul(big.NewInt(3), nutil.BigIntPow10(50)),
		},
	}
	...
	for _, testCase := range testCases {
		...
		t.Run(fmt.Sprintf(`bigInt: %s, sqrtBigInt: %s`, tc.bigInt, tc.sqrtBigInt), func(t *testing.T) {
			sqrtInt, err := nutil.SqrtBigInt(tc.bigInt)
...
func TestSqrtDec(t *testing.T) {
	...
	t.Run("negative sqrt should panic", func(t *testing.T) {
		panicString := nutil.TryCatch(func() {
			nutil.MustSqrtDec(sdkmath.LegacyNewDec(-9))
		})().Error()
	...
	for _, testCase := range testCases {
		...
		t.Run(fmt.Sprintf(`dec: %s, sqrtDec: %s`, tc.dec, tc.sqrtDec), func(t *testing.T) {
			sqrtDec, err := nutil.SqrtDec(tc.dec)
			assert.NoError(t, err)
...
func TestBankersRound(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		rounded := nutil.BankersRound(tcQuo, tc.rem, halfPrecision)
...
func TestClamp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.description, func(t *testing.T) {
			result := nutil.Clamp(tt.value, tt.clampValue)
