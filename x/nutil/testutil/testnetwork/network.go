...
func New(testPtr *suite.Suite, cfg Config) (network *Network) {
	...
	defer func() {
		...
		if network == nil {
			panic("Mistakes were made. *Network is nil even though theres' no error.")
		}
	}()
	...
	for idx, v := range network.Validators {
		err := startNodeAndServers(cfg, v, uint(idx))
		if err != nil {
...
// WaitForNextBlockVerbose waits for the next block to be committed, returning an
// error upon failure.
