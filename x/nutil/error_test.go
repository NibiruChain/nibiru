...
func TestCombineErrors(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			errOut := nutil.CombineErrors(tc.errs...)
...
func TestCombineErrorsGeneric(t *testing.T) {
	testCases := []struct {
		...
		// cases: fmt.Stringer
		{name: "type=fmt.Stringer |", in: oracletypes.NewPair(denoms.USDC, denoms.NUSD), out: errors.New("uusdc:unusd")},

		// cases: []fmt.Stringer
		{
			name: "type=[]fmt.Stringer | happy",
			in: []fmt.Stringer{
				oracletypes.NewPair(denoms.BTC, denoms.NUSD),
				oracletypes.NewPair(denoms.ETH, denoms.NUSD),
			},
	...
	for _, testCase := range testCases {
		tc := testCase
		t.Run(tc.name, func(t *testing.T) {
			out, ok := nutil.CombineErrorsGeneric(tc.in)
			if tc.fail {
	...
	testCases := []testutil.FunctionTestCase{
		{
			...
			Test: func() {
				...
				out, ok := nutil.ToError(description)
				assert.True(t, ok)
		...
		},
		{
			...
			Test: func() {
				description := "an error description"
				out, ok := nutil.ToError(errors.New(description))
				assert.True(t, ok)
				assert.EqualValues(t, out.Error(), description)
			},
		},
		...
		},
		{
			...
			Test: func() {
				...
				_, ok := nutil.ToError(descriptionOfBadType)
				assert.False(t, ok)
			},
		},
		{
		...
		},
		{
			...
			Test: func() {
				err, ok := nutil.ToError([]string{"abc", "123"})
				assert.True(t, ok)
		...
		},
		{
			Name: "slice of error",
			Test: func() {
				err, ok := nutil.ToError(newErrors("abc", "123"))
				assert.True(t, ok)
				assert.Equal(t, errors.New("abc: 123"), err)
			},
		},
