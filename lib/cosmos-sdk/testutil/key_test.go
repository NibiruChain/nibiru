...
func TestGenerateCoinKey(t *testing.T) {
	...
	// Test creation
	k, err := keyring.NewInMemory(cdc).NewAccount("xxx", mnemonic, "", hd.NewFundraiserParams(0, sdk.GetConfig().GetCoinType(), 0).String(), hd.Secp256k1)
	require.NoError(t, err)
...
func TestGenerateSaveCoinKey(t *testing.T) {
	...
	k, err = keyring.NewInMemory(encCfg.Codec).NewAccount("xxx", mnemonic, "", hd.NewFundraiserParams(0, sdk.GetConfig().GetCoinType(), 0).String(), hd.Secp256k1)
	require.NoError(t, err)
	addr1, err = k.GetAddress()
	require.NoError(t, err)
	require.Equal(t, addr, addr1)
