		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if pkStr, _ := cmd.Flags().GetString(cli.FlagPubKey); pkStr != "" {
				if err := clientCtx.Codec.UnmarshalInterfaceJSON([]byte(pkStr), &valPubKey); err != nil {
					return pkgerrors.Wrap(err, "failed to unmarshal validator public key")
				}
			}

			genDoc, err := cmttypes.GenesisDocFromFile(config.GenesisFile())
			if err != nil {
				return pkgerrors.Wrapf(err, "failed to read genesis doc file %s", config.GenesisFile())
			}

			var genesisState map[string]json.RawMessage
			if err = json.Unmarshal(genDoc.AppState, &genesisState); err != nil {
				return pkgerrors.Wrap(err, "failed to unmarshal genesis state")
			}

			if err = mbm.ValidateGenesis(cdc, txEncCfg, genesisState); err != nil {
				return pkgerrors.Wrap(err, "failed to validate genesis state")
			}
			...
			if err != nil {
				return pkgerrors.Wrapf(err, "failed to fetch '%s' from the keyring", name)
			}
			...
			if err != nil {
				return pkgerrors.Wrap(err, "error creating configuration to create validator msg")
			}

			amount := args[1]
			coins, err := sdk.ParseCoinsNormalized(amount)
			if err != nil {
				return pkgerrors.Wrap(err, "failed to parse coins")
			}
			...
			if err != nil {
				return pkgerrors.Wrap(err, "failed to validate account in genesis")
			}
			...
			if err != nil {
				return pkgerrors.Wrap(err, "failed to build create-validator message")
			}
			...
			if err = txBldr.PrintUnsignedTx(clientCtx, msg); err != nil {
				return pkgerrors.Wrap(err, "failed to print unsigned std tx")
			}

			// read the transaction
			stdTx, err := readUnsignedGenTxFile(clientCtx, w)
			if err != nil {
				return pkgerrors.Wrap(err, "failed to read unsigned gen tx file")
			}
			...
			if err != nil {
				return pkgerrors.Wrap(err, "failed to sign std tx")
			}
			...
			if outputDocument == "" {
				...
				if err != nil {
					return pkgerrors.Wrap(err, "failed to create output file path")
				}
			}

			if err := writeSignedGenTx(clientCtx, outputDocument, stdTx); err != nil {
				return pkgerrors.Wrap(err, "failed to write signed gen tx")
			}
