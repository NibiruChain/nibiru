...
func (k Keeper) storeWasmCode(ctx sdk.Context, code []byte, storeFn func(code wasmvm.WasmCode) (wasmvm.Checksum, error)) ([]byte, error) {
	...
	if !bytes.Equal(vmChecksum, checksum) {
		return nil, sdkioerrors.Wrapf(types.ErrInvalidChecksum, "expected %s, got %s", hex.EncodeToString(checksum), hex.EncodeToString(vmChecksum))
	}

	// pin the code to the vm in-memory cache
	if err := ibcwasm.GetVM().Pin(vmChecksum); err != nil {
		return nil, sdkioerrors.Wrapf(err, "failed to pin contract with checksum (%s) to vm cache", hex.EncodeToString(vmChecksum))
	}

	err = types.AddChecksum(ctx, k.cdc, ibcwasm.GetWasmStoreKey(), checksum)
	if err != nil {
		return nil, sdkioerrors.Wrap(err, "failed to store checksum")
	}
...
func (k Keeper) migrateContractCode(ctx sdk.Context, clientID string, newChecksum, migrateMsg []byte) error {
	...
	if err != nil {
		return sdkioerrors.Wrap(err, "failed to retrieve wasm client state")
	}
	...
	if err != nil {
		return sdkioerrors.Wrap(err, "contract migration failed")
	}
	...
	if err != nil {
		...
		return sdkioerrors.Wrap(err, "failed to retrieve the updated wasm client state")
	}
...
func (k Keeper) GetWasmClientState(ctx sdk.Context, clientID string) (*types.ClientState, error) {
	...
	if !found {
		return nil, sdkioerrors.Wrapf(clienttypes.ErrClientTypeNotFound, "clientID %s", clientID)
	}

	wasmClientState, ok := clientState.(*types.ClientState)
	if !ok {
		return nil, sdkioerrors.Wrapf(clienttypes.ErrInvalidClient, "expected type %T, got %T", (*types.ClientState)(nil), wasmClientState)
	}
