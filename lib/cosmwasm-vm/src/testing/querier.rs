...
use cosmwasm_std::testing::{
    MockQuerier as StdMockQuerier, MockQuerierCustomHandlerResult,
};
use cosmwasm_std::{
    to_json_binary, to_json_vec, Binary, Coin, ContractResult, CustomQuery,
    Empty, Querier as _, QueryRequest, SystemError, SystemResult,
};
...
impl<C: CustomQuery + DeserializeOwned> Querier for MockQuerier<C> {
    fn query_raw(
        ...
        let gas_info = GasInfo::with_externally_used(
            GAS_COST_QUERY_FLAT
                + (GAS_COST_QUERY_REQUEST_MULTIPLIER
                    * (bin_request.len() as u64))
...
impl MockQuerier {
    pub fn query<C: CustomQuery>(
        ...
        let request_binary = match to_json_vec(request) {
            ...
            Err(err) => {
                let gas_info =
                    GasInfo::with_externally_used(err.to_string().len() as u64);
                return (
...
mod tests {
    use super::*;
    use cosmwasm_std::{
        coin, from_json, AllBalanceResponse, BalanceResponse, BankQuery, Empty,
    };
    ...
    fn query_raw_fails_when_out_of_gas() {
        ...
        let (result, _gas_info) =
            querier.query_raw(b"broken request", gas_limit);
