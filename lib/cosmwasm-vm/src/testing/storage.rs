    ...
    fn scan(
        ...
        let values: Vec<Record> =
            match (bounds.start_bound(), bounds.end_bound()) {
                ...
                _ => match order {
                    Order::Ascending => {
                        self.data.range(bounds).map(clone_item).collect()
                    }
                    Order::Descending => {
                        self.data.range(bounds).rev().map(clone_item).collect()
                    }
                },
            };
...
mod tests {
    ...
    fn iterator() {
        ...
        {
            let iter_id =
                store.scan(Some(b"f"), None, Order::Ascending).0.unwrap();
            let elements = store.all(iter_id).0.unwrap();
            assert_eq!(
                elements,
                vec![
        ...
        {
            let iter_id =
                store.scan(Some(b"f"), None, Order::Descending).0.unwrap();
            let elements = store.all(iter_id).0.unwrap();
            assert_eq!(
                elements,
                vec![
        ...
        {
            let iter_id =
                store.scan(None, Some(b"f"), Order::Ascending).0.unwrap();
            let elements = store.all(iter_id).0.unwrap();
        ...
        {
            let iter_id =
                store.scan(None, Some(b"no"), Order::Descending).0.unwrap();
            let elements = store.all(iter_id).0.unwrap();
            assert_eq!(
                elements,
                vec![
