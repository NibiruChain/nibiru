...
use wasmer::{
    AsStoreMut, Instance as WasmerInstance, Memory, MemoryView, Value,
};
...
//                            |                                                 |
//                            v                                                 v
pub type DebugHandlerFn =
    dyn for<'a, 'b> FnMut(/* msg */ &'a str, DebugInfo<'b>);
...
unsafe impl<A: BackendApi, S: Storage, Q: Querier> Send
    for Environment<A, S, Q>
{
}

unsafe impl<A: BackendApi, S: Storage, Q: Querier> Sync
    for Environment<A, S, Q>
{
}
...
impl<A: BackendApi, S: Storage, Q: Querier> Environment<A, S, Q> {
    ...
    {
        self.with_context_data_mut(|context_data| {
            callback(&mut context_data.gas_state)
        })
    }
    ...
    {
        self.with_context_data(|context_data| {
            match context_data.wasmer_instance {
                Some(instance_ptr) => {
    ...
    {
        self.with_context_data_mut(|context_data| {
            match context_data.querier.as_mut() {
...
mod tests {
    ...
    use cosmwasm_std::{
        coins, from_json, to_json_vec, AllBalanceResponse, BankQuery, Empty,
        QueryRequest,
    };
    ...
    fn process_gas_info_works_for_cost() {
        ...
        match process_gas_info(&env, &mut store, GasInfo::with_cost(1))
            .unwrap_err()
        {
    ...
    fn process_gas_info_works_for_externally_used() {
        ...
        // Consume all the Gas that we allocated
        process_gas_info(&env, &mut store, GasInfo::with_externally_used(70))
            .unwrap();
        assert_eq!(env.get_gas_left(&mut store), 30);
        process_gas_info(&env, &mut store, GasInfo::with_externally_used(4))
            .unwrap();
        assert_eq!(env.get_gas_left(&mut store), 26);
        process_gas_info(&env, &mut store, GasInfo::with_externally_used(6))
            .unwrap();
        assert_eq!(env.get_gas_left(&mut store), 20);
        process_gas_info(&env, &mut store, GasInfo::with_externally_used(20))
            .unwrap();
        assert_eq!(env.get_gas_left(&mut store), 0);
        ...
        match process_gas_info(
            &env,
            &mut store,
            GasInfo::with_externally_used(1),
        )
        .unwrap_err()
        {
            VmError::GasDepletion { .. } => {}
    ...
    fn process_gas_info_zeros_gas_left_when_exceeded() {
        ...
        {
            let (env, mut store, _instance) = make_instance(100);
            let result = process_gas_info(
                &env,
                &mut store,
                GasInfo::with_externally_used(120),
            );
        ...
        // with_cost
        {
            let (env, mut store, _instance) = make_instance(100);
            let result =
                process_gas_info(&env, &mut store, GasInfo::with_cost(120));
            match result.unwrap_err() {
                VmError::GasDepletion { .. } => {}
                err => panic!("unexpected error: {err:?}"),
            }
    ...
    fn process_gas_info_works_correctly_with_gas_consumption_in_wasmer() {
        ...
        process_gas_info(&env, &mut store, GasInfo::with_externally_used(50))
            .unwrap();
    ...
    fn call_function_fails_for_missing_instance() {
        ...
        match res.unwrap_err() {
            VmError::UninitializedContextData { kind, .. } => {
    ...
    fn call_function_fails_for_missing_function() {
        ...
        match res.unwrap_err() {
            VmError::ResolveErr { msg, .. } => {
                assert_eq!(
                    msg,
                    "Could not get export: Missing export doesnt_exist"
                );
            }
            err => panic!("Unexpected error: {err:?}"),
        }
    ...
    fn with_querier_from_context_works() {
        ...
        let res = env
            .with_querier_from_context::<_, _>(|querier| {
                ...
                let (result, _gas_info) = querier.query_raw(
                    &to_json_vec(&req).unwrap(),
