...
use wasmer::wasmparser::{
    BinaryReaderError, CompositeInnerType, Export, FuncToValidate, FunctionBody,
    Import, MemoryType, Parser, Payload, TableType, ValidPayload, Validator,
...
pub enum FunctionValidator<'a> {
    Pending(
        OpaqueDebug<Vec<(FuncToValidate<ValidatorResources>, FunctionBody<'a>)>>,
    ),
    Success,
...
impl<'a> FunctionValidator<'a> {
    fn push(
        &mut self,
        item: (FuncToValidate<ValidatorResources>, FunctionBody<'a>),
    ) {
        ...
        for p in Parser::new(0).parse_all(wasm) {
            ...
            match p {
                        ...
                        for ty in types {
                            match ty.composite_type.inner {
                                CompositeInnerType::Func(ft) => {
                                    ...
                                    this.max_func_params = core::cmp::max(
                                        ft.params().len(),
                                        this.max_func_params,
                                    );
                                    this.max_func_results = core::cmp::max(
                                        ft.results().len(),
                ...
                Payload::ImportSection(i) => {
                    this.imports =
                        i.into_iter().collect::<Result<Vec<_>, _>>()?;
                }
                ...
                Payload::MemorySection(m) => {
                    this.memories =
                        m.into_iter().collect::<Result<Vec<_>, _>>()?;
                }
                Payload::ExportSection(e) => {
                    this.exports =
                        e.into_iter().collect::<Result<Vec<_>, _>>()?;
                }
                    ...
                    if reader.name() == "cw_migrate_version" =>
                {
                    ...
                    let raw_version =
                        str::from_utf8(reader.data()).map_err(|err| {
                            VmError::static_validation_err(err.to_string())
                        })?;

                    this.contract_migrate_version =
                        Some(raw_version.parse::<u64>().map_err(|err| {
                            VmError::static_validation_err(err.to_string())
                        })?);
                }
...
mod test {
    ...
    fn read_migrate_version() {
        let wasm_data = wat::parse_str(
            r#"( module ( @custom "cw_migrate_version" "42" ) )"#,
        )
        .unwrap();
    ...
    fn read_migrate_version_fails() {
        let wasm_data = wat::parse_str(
            r#"( module ( @custom "cw_migrate_version" "not a number" ) )"#,
        )
        .unwrap();
