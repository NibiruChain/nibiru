...
impl FileSystemCache {
    ...
    pub unsafe fn new(
        ...
        if base_path.exists() {
            ...
            mkdir_p(&base_path)
                .map_err(|_e| NewFileSystemCacheError::CouldntCreatePath)?;
        }
    ...
    pub fn load(
        ...
        let result = if self.unchecked_modules {
            unsafe {
                Module::deserialize_from_file_unchecked(&engine, &file_path)
            }
        } else {
    ...
    pub fn store(
        ...
        mkdir_p(&self.modules_path).map_err(|_e| {
            VmError::cache_err("Error creating modules directory")
        })?;

        let path = self.module_file(checksum);
        catch_unwind(|| {
            module.serialize_to_file(&path).map_err(|e| {
                VmError::cache_err(format!("Error writing module to disk: {e}"))
            })
        })
    ...
    pub fn remove(&mut self, checksum: &Checksum) -> VmResult<bool> {
        ...
        if file_path.exists() {
            fs::remove_file(file_path).map_err(|_e| {
                VmError::cache_err("Error deleting module from disk")
            })?;
            Ok(true)
        } else {
            Ok(false)
...
fn modules_path(
    ...
    let version_dir =
        format!("{MODULE_SERIALIZATION_VERSION}-wasmer{wasmer_module_version}");
...
mod tests {
    ...
    fn file_system_cache_run() {
        ...
        let mut cache =
            unsafe { FileSystemCache::new(tmp_dir.path(), false).unwrap() };
        ...
        // This is not really testing the cache API but better safe than sorry.
        {
            ...
            let instance =
                WasmerInstance::new(&mut store, &cached_module, &import_object)
                    .unwrap();
    ...
    fn target_id_works() {
        ...
        let target =
            Target::new(triple.clone(), wasmer::CpuFeature::POPCNT.into());
