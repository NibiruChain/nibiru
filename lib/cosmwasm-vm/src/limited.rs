...
fn collection_to_string_limited<
    E: AsRef<str>,
...
>(
    ...
    if out.len() + closing.len() <= max_length {
        ...
        loop {
            let previous_length = lengths_stack.pop().expect(
                "Cannot remove hide enough elements to fit in length limit.",
            );
...
mod tests {
    ...
    #[test]
    #[should_panic(
        expected = "Cannot remove hide enough elements to fit in length limit."
    )]
