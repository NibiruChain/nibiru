...
pub fn decode_sections(data: &[u8]) -> Result<Vec<&[u8]>, CommunicationError> {
    ...
    while remaining_len >= 4 {
        ...
        let section_start =
            tail_len_idx.checked_sub(tail_len).ok_or_else(|| {
                CommunicationError::invalid_section("section length overflow")
            })?;
...
mod tests {
    ...
    fn encode_sections_works_for_multiple_elements() {
        ...
        let enc =
            encode_sections(&[vec![0xAA], vec![0xDE, 0xDE], vec![]]).unwrap();
