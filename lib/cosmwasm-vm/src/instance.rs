...
use wasmer::{
    Exports, Function, FunctionEnv, Imports, Instance as WasmerInstance, Module,
    Store, Value,
};
...
use crate::imports::{
    do_abort, do_addr_canonicalize, do_addr_humanize, do_addr_validate,
    do_db_read, do_db_remove, do_db_write, do_debug, do_ed25519_batch_verify,
    do_ed25519_verify, do_query_chain, do_secp256k1_recover_pubkey,
    do_secp256k1_verify,
};
    ...
    Q: Querier + 'static, // 'static is needed here to allow using this in an Environment that is cloned into closures
{
    ...
    pub(crate) fn from_module(
        ...
        let wasmer_instance = Box::from(
            ...
            .map_err(|original| {
                VmError::instantiation_err(format!(
                    "Error instantiating module: {original}"
                ))
            })?,
        );
        ...
        let memory = wasmer_instance
            ...
            .map_err(|original| {
                VmError::instantiation_err(format!(
                    "Could not get memory 'memory': {original}"
                ))
            })?
            .clone();
...
mod tests {
    ...
    use crate::testing::{
        ...
        mock_instance_with_balances, mock_instance_with_failing_api,
        mock_instance_with_gas_limit, mock_instance_with_options,
        MockInstanceOptions,
    };
    use cosmwasm_std::{
        coin, coins, from_json, AllBalanceResponse, BalanceResponse, BankQuery,
        Empty, QueryRequest,
    };
    ...
    fn set_debug_handler_and_unset_debug_handler_work() {
        ...
        eprintln!(
            "Unsetting debug handler. From here nothing is printed anymore."
        );
    ...
    fn required_capabilities_works_for_many_exports() {
        ...
        let (instance_options, memory_limit) = mock_instance_options();
        let instance =
            Instance::from_code(&wasm, backend, instance_options, memory_limit)
                .unwrap();
    ...
    fn extra_imports_get_added() {
        ...
        let fun = Function::new_typed_with_env(
            &mut store,
            &fe,
            move |fe_mut: FunctionEnvMut<MyEnv>| {
    ...
    fn errors_in_imports() {
        ...
        let mut instance =
            mock_instance_with_failing_api(CONTRACT, &[], error_message);
    ...
    fn with_querier_works_readonly() {
        ...
        let mut instance = mock_instance_with_balances(
            CONTRACT,
            &[(&rich_addr, &rich_balance)],
        );
    ...
    fn query_works_with_gas_metering() {
        ...
        let info = mock_info("creator", &coins(1000, "earth"));
        let msg = br#"{"verifier": "verifies", "beneficiary": "benefits"}"#;
        let _res = call_instantiate::<_, _, _, Empty>(
            &mut instance,
            &mock_env(),
            &info,
            msg,
