    ...
    T: Deserialize<'a>,
{
    ...
    serde_json::from_slice(value)
        .map_err(|e| VmError::parse_err(type_name::<T>(), e))
}
    ...
    T: Serialize + ?Sized,
{
    serde_json::to_vec(data)
        .map_err(|e| VmError::serialize_err(type_name::<T>(), e))
}
...
mod tests {
    ...
    fn from_slice_works() {
        let deserialized: SomeMsg =
            from_slice(br#"{"refund":{}}"#, LIMIT).unwrap();
    ...
    fn from_slice_works_for_special_chars() {
        let deserialized: SomeMsg =
            from_slice(br#"{"cowsay":{"text":"foo\"bar\\\"bla"}}"#, LIMIT)
                .unwrap();
        assert_eq!(
            deserialized,
            SomeMsg::Cowsay {
