...
fn check_interface_version(module: &ParsedWasm) -> VmResult<()> {
    let mut interface_version_exports = module
        ...
        .into_iter();
    if let Some(first_interface_version_export) =
        interface_version_exports.next()
    {
...
fn check_wasm_exports(module: &ParsedWasm) -> VmResult<()> {
    let available_exports: HashSet<String> =
        module.exported_function_names(None);
...
mod tests {
    ...
    static CONTRACT_0_12: &[u8] =
        include_bytes!("../testdata/hackatom_0.12.wasm");
    static CONTRACT_0_14: &[u8] =
        include_bytes!("../testdata/hackatom_0.14.wasm");
    static CONTRACT_0_15: &[u8] =
        include_bytes!("../testdata/hackatom_0.15.wasm");
    static CONTRACT: &[u8] = include_bytes!("../testdata/hackatom.wasm");
    static CONTRACT_RUST_170: &[u8] =
        include_bytes!("../testdata/cyberpunk_rust170.wasm");
    ...
    fn check_wasm_tables_works() {
        ...
        // One table (bound)
        let wasm =
            wat::parse_str("(module (table $name 123 123 funcref))").unwrap();
        check_wasm_tables(&ParsedWasm::parse(&wasm).unwrap()).unwrap();

        // One table (bound, initial > max)
        let wasm =
            wat::parse_str("(module (table $name 124 123 funcref))").unwrap();
        ...
        let wasm =
            wat::parse_str("(module (table $name 100 9999 funcref))").unwrap();
        let err =
            check_wasm_tables(&ParsedWasm::parse(&wasm).unwrap()).unwrap_err();
    ...
    fn check_wasm_memories_initial_size() {
        ...
        match check_wasm_memories(&ParsedWasm::parse(&wasm_too_big).unwrap()) {
            Err(VmError::StaticValidationErr { msg, .. }) => {
                assert!(msg.starts_with(
                    "Wasm contract memory's minimum must not exceed 512 pages"
                ));
            }
            Err(e) => panic!("Unexpected error {e:?}"),
            Ok(_) => panic!("Didn't reject wasm with invalid api"),
    ...
    fn check_wasm_memories_maximum_size() {
        ...
        match check_wasm_memories(&ParsedWasm::parse(&wasm_max).unwrap()) {
            Err(VmError::StaticValidationErr { msg, .. }) => {
                assert!(msg.starts_with(
                    "Wasm contract memory's maximum must be unset"
                ));
            }
            Err(e) => panic!("Unexpected error {e:?}"),
            Ok(_) => panic!("Didn't reject wasm with invalid api"),
    ...
    fn check_wasm_exports_works() {
        ...
        match check_wasm_exports(&module) {
            Err(VmError::StaticValidationErr { msg, .. }) => {
                assert!(msg.starts_with(
                    "Wasm contract doesn't have required export: \"allocate\""
                ));
            }
            Err(e) => panic!("Unexpected error {e:?}"),
            Ok(_) => panic!("Didn't reject wasm with invalid api"),
        ...
        match check_wasm_exports(&module) {
            Err(VmError::StaticValidationErr { msg, .. }) => {
                assert!(msg.starts_with(
                    "Wasm contract doesn't have required export: \"deallocate\""
                ));
            }
            Err(e) => panic!("Unexpected error {e:?}"),
            Ok(_) => panic!("Didn't reject wasm with invalid api"),
    ...
    fn check_wasm_imports_exceeds_limit() {
        ...
        match err {
            VmError::StaticValidationErr { msg, .. } => {
                assert_eq!(
                    msg,
                    "Import count exceeds limit. Imports: 101. Limit: 100."
                );
            }
            err => panic!("Unexpected error: {err:?}"),
        }
    ...
    fn check_wasm_imports_of_old_contract() {
        ...
        match result.unwrap_err() {
            VmError::StaticValidationErr { msg, .. } => {
                assert!(msg.starts_with(
                    "Wasm contract requires unsupported import: \"env.read_db\""
                ));
            }
            err => panic!("Unexpected error: {err:?}"),
        }
    ...
    fn check_wasm_imports_wrong_type() {
        let wasm =
            wat::parse_str(r#"(module (import "env" "db_read" (memory 1 1)))"#)
                .unwrap();
        let result = check_wasm_imports(
            &ParsedWasm::parse(&wasm).unwrap(),
            SUPPORTED_IMPORTS,
