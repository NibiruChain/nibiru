...
use cosmwasm_std::{
    ContractResult, CustomMsg, Env, MessageInfo, QueryResponse, Reply, Response,
};
#[cfg(feature = "stargate")]
use cosmwasm_std::{
    Ibc3ChannelOpenResponse, IbcBasicResponse, IbcChannelCloseMsg,
        ...
        serde_json::from_slice::<serde_json::Value>(binary_response.as_slice())
            .map_err(|e| {
                VmError::generic_err(format!(
                    "Query response must be valid JSON. {e}"
                ))
            })?;
    }
    ...
    U: DeserializeOwned + CustomMsg,
{
    ...
    let result =
        from_slice(&data, deserialization_limits::RESULT_IBC_CHANNEL_CONNECT)?;
    Ok(result)
}
    ...
    U: DeserializeOwned + CustomMsg,
{
    ...
    let result =
        from_slice(&data, deserialization_limits::RESULT_IBC_CHANNEL_CLOSE)?;
    Ok(result)
}
    ...
    U: DeserializeOwned + CustomMsg,
{
    ...
    let result =
        from_slice(&data, deserialization_limits::RESULT_IBC_PACKET_ACK)?;
    Ok(result)
}
    ...
    U: DeserializeOwned + CustomMsg,
{
    ...
    let result =
        from_slice(&data, deserialization_limits::RESULT_IBC_PACKET_TIMEOUT)?;
    Ok(result)
}
...
mod tests {
    use super::*;
    use crate::testing::{
        mock_env, mock_info, mock_instance, mock_instance_with_options,
        MockInstanceOptions,
    };
    ...
    fn call_execute_runs_out_of_gas() {
        ...
        let msg = br#"{"cpu_loop":{}}"#;
        let err = call_execute::<_, _, _, Empty>(
            &mut instance,
            &mock_env(),
            &info,
            msg,
    ...
    fn call_migrate_works() {
        ...
        let _res =
            call_migrate::<_, _, _, Empty>(&mut instance, &mock_env(), msg);

        // query the new_verifier with verifier
        let msg = br#"{"verifier":{}}"#;
        let contract_result =
            call_query(&mut instance, &mock_env(), msg).unwrap();
    ...
    fn float_instrs_are_deterministic() {
        ...
        for instr in &instructions {
            for seed in 0..RUNS_PER_INSTRUCTION {
                ...
                let result =
                    match call_query(&mut instance, &mock_env(), msg.as_bytes())
                    {
                        Ok(ContractResult::Ok(r)) => {
                            format!("{:?}", from_json::<Value>(&r).unwrap())
                        }
    ...
    mod ibc {
        ...
        use crate::testing::{
            mock_env, mock_info, mock_instance, MockApi, MockQuerier,
            MockStorage,
        };
        use cosmwasm_std::testing::{
            mock_ibc_channel_close_init, mock_ibc_channel_connect_ack,
            mock_ibc_channel_open_init, mock_ibc_packet_ack,
            mock_ibc_packet_recv, mock_ibc_packet_timeout, mock_wasmd_attr,
        };
        use cosmwasm_std::{
            Empty, Event, IbcAcknowledgement, IbcOrder, Reply, ReplyOn,
        ...
        fn setup(
            ...
            let handshake_connect = mock_ibc_channel_connect_ack(
                channel_id,
                IbcOrder::Ordered,
                IBC_VERSION,
            );
            ...
            call_reply::<_, _, _, Empty>(instance, &mock_env(), &response)
                .unwrap();
        }
        ...
        fn call_ibc_channel_close_works() {
            let mut instance = mock_instance(CONTRACT, &[]);
            setup(&mut instance, CHANNEL_ID, ACCOUNT);
            let handshake_close = mock_ibc_channel_close_init(
                CHANNEL_ID,
                IbcOrder::Ordered,
                IBC_VERSION,
            );
        ...
        fn call_ibc_packet_ack_works() {
            ...
            call_ibc_packet_ack::<_, _, _, Empty>(
                &mut instance,
                &mock_env(),
                &msg,
            )
        ...
        fn call_ibc_packet_timeout_works() {
            ...
            call_ibc_packet_timeout::<_, _, _, Empty>(
                &mut instance,
                &mock_env(),
                &msg,
            )
        ...
        fn call_ibc_packet_receive_works() {
            ...
            call_ibc_packet_receive::<_, _, _, Empty>(
                &mut instance,
                &mock_env(),
                &msg,
            )
