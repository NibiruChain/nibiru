...
use cosmwasm_vm::testing::{
    mock_backend, mock_env, mock_info, mock_instance_options, MockApi,
    MockQuerier, MockStorage,
};
use cosmwasm_vm::{
    call_execute, call_instantiate, capabilities_from_csv, Cache, CacheOptions,
    Checksum, Instance, InstanceOptions, Size,
};
...
fn bench_instance(c: &mut Criterion) {
    ...
    group.bench_function("execute execute (release)", |b| {
        ...
        b.iter(|| {
            ...
            let msg = br#"{"release":{}}"#;
            let contract_result = call_execute::<_, _, _, Empty>(
                &mut instance,
                &mock_env(),
                &info,
                msg,
...
pub fn bench_instance_threads(c: &mut Criterion) {
    c.bench_function("multi-threaded get_instance", |b| {
        ...
        let mut random_checksum = || {
            ...
            leb128::write::unsigned(&mut writable, r)
