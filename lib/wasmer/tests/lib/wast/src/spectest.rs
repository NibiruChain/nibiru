...
pub fn spectest_importobject(store: &mut Store) -> Imports {
    ...
    let print_i32 =
        Function::new_typed(store, |val: i32| println!("{}: i32", val));
    let print_i64 =
        Function::new_typed(store, |val: i64| println!("{}: i64", val));
    let print_f32 =
        Function::new_typed(store, |val: f32| println!("{}: f32", val));
    let print_f64 =
        Function::new_typed(store, |val: f64| println!("{}: f64", val));
    ...
    let global_f64 =
        Global::new(store, Value::F64(f64::from_bits(0x4084_d000_0000_0000)));
