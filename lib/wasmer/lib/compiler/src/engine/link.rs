...
fn apply_relocation(
    ...
    let target_func_address: usize = match r.reloc_target() {
        RelocationTarget::LocalFunc(index) => {
            *allocated_functions[index].ptr as usize
        }
        RelocationTarget::LibCall(libcall) => {
            ...
            if r.kind() == RelocationKind::Abs8
                || r.kind() == RelocationKind::X86PCRel8
            {
    ...
    match r.kind() {
        RelocationKind::Abs8 => unsafe {
            let (reloc_address, reloc_delta) =
                r.for_address(body, target_func_address as u64);
        ...
        RelocationKind::Arm64Movw0 => unsafe {
            let (reloc_address, reloc_delta) =
                r.for_address(body, target_func_address as u64);
            let reloc_delta = (((reloc_delta & 0xffff) as u32) << 5)
                | read_unaligned(reloc_address as *mut u32);
            write_unaligned(reloc_address as *mut u32, reloc_delta);
        },
        ...
        RelocationKind::RiscvPCRelHi20 => unsafe {
            ...
            let reloc_delta = ((reloc_delta.wrapping_add(0x800) & 0xfffff000)
                as u32)
                | read_unaligned(reloc_address as *mut u32);
            write_unaligned(reloc_address as *mut u32, reloc_delta);
        },
        RelocationKind::RiscvPCRelLo12I => unsafe {
            let (reloc_address, reloc_abs) =
                r.for_address(body, target_func_address as u64);
        ...
        RelocationKind::LArchAbsLo12 | RelocationKind::LArchPCAlaLo12 => unsafe {
            let (reloc_address, reloc_abs) =
                r.for_address(body, target_func_address as u64);
            let reloc_abs = (((reloc_abs & 0xfff) as u32) << 10)
                | read_unaligned(reloc_address as *mut u32);
            write_unaligned(reloc_address as *mut u32, reloc_abs);
        },
