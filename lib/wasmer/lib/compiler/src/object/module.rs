...
use object::{
    elf, macho,
    write::{
        Object, Relocation, StandardSection, StandardSegment,
        Symbol as ObjSymbol, SymbolSection,
    },
    FileFlags, RelocationEncoding, RelocationKind, SectionKind, SymbolFlags,
...
pub fn get_object_for_target(triple: &Triple) -> Result<Object, ObjectError> {
    ...
    let mut object =
        Object::new(obj_binary_format, obj_architecture, obj_endianness);
...
pub fn emit_compilation(
    ...
    let custom_section_ids = compilation
        ...
        .map(|(section_index, custom_section)| {
            if debug_index.map_or(false, |d| d == section_index) {
                ...
                let section_name = symbol_registry
                    .symbol_to_name(Symbol::Section(section_index));
                let symbol_id = obj.add_symbol(ObjSymbol {
                ...
                let section_name = symbol_registry
                    .symbol_to_name(Symbol::Section(section_index));
                let (section_kind, standard_section) =
                    match custom_section.protection {
    ...
    for (signature_index, function) in
        compilation.function_call_trampolines.into_iter()
    {
        let function_name = symbol_registry
            .symbol_to_name(Symbol::FunctionCallTrampoline(signature_index));
        let section_id = obj.section_id(StandardSection::Text);
        let symbol_id = obj.add_symbol(ObjSymbol {
            name: function_name.into_bytes(),
            value: 0,
    ...
    for (func_index, function) in
        compilation.dynamic_function_trampolines.into_iter()
    {
        let function_name = symbol_registry
    ...
    for (function_local_index, relocations) in function_relocations.into_iter() {
        let (section_id, symbol_id) =
            function_symbol_ids.get(function_local_index).unwrap();
    ...
    for (section_index, relocations) in custom_section_relocations.into_iter() {
        if !debug_index.map_or(false, |d| d == section_index) {
            ...
            let (section_id, symbol_id) =
                custom_section_ids.get(section_index).unwrap();
            all_relocations.push((*section_id, *symbol_id, relocations));
        }
    }

    for (section_id, symbol_id, relocations) in all_relocations.into_iter() {
        let (_symbol_id, section_offset) =
            obj.symbol_section_and_offset(symbol_id).unwrap();

        for r in relocations {
            let relocation_address = section_offset + r.offset as u64;

            let (relocation_kind, relocation_encoding, relocation_size) = match r
                .kind
            {
                Reloc::Abs4 => {
            ...
            match r.reloc_target {
                ...
                RelocationTarget::LibCall(libcall) => {
                    ...
                    let target_symbol =
                        obj.symbol_id(libcall_fn_name).unwrap_or_else(|| {
