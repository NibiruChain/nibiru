...
use cosmwasm_crypto::{
    ed25519_batch_verify, ed25519_verify, secp256k1_recover_pubkey,
...
>(
    ...
    let value =
        read_region(&data.memory(&store), value_ptr, MAX_LENGTH_DB_VALUE)
            .map_err(|e| convert_error(e, "Value"))?;

    let (result, gas_info) =
        data.with_storage_from_context::<_, _>(|store| {
...
>(
    ...
    let source_data =
        read_region(&data.memory(&store), source_ptr, MAX_LENGTH_HUMAN_ADDRESS)?;
...
>(
    ...
    let hash =
        read_region(&data.memory(&store), hash_ptr, MESSAGE_HASH_MAX_LEN)?;
    let signature =
        read_region(&data.memory(&store), signature_ptr, ECDSA_SIGNATURE_LEN)?;
    let pubkey =
        read_region(&data.memory(&store), pubkey_ptr, ECDSA_PUBKEY_MAX_LEN)?;
...
>(
    ...
    let recover_param: u8 = match recover_param.try_into() {
        ...
        Err(_) => {
            return Ok(
                (CryptoError::invalid_recovery_param().code() as u64) << 32
            )
        }
    };

    let gas_info =
        GasInfo::with_cost(data.gas_config.secp256k1_recover_pubkey_cost);
    process_gas_info(data, &mut store, gas_info)?;
    let result = secp256k1_recover_pubkey(&hash, &signature, recover_param);
    match result {
        Ok(pubkey) => {
            let pubkey_ptr =
                write_to_contract(data, &mut store, pubkey.as_ref())?;
...
>(
    ...
    let pubkey =
        read_region(&data.memory(&store), pubkey_ptr, EDDSA_PUBKEY_LEN)?;
...
>(
    ...
    let gas_info =
        GasInfo::with_cost(max(gas_cost, data.gas_config.ed25519_verify_cost));
    process_gas_info(data, &mut store, gas_info)?;
...
>(
    ...
    let gas_remaining = data.get_gas_left(&mut store);
    let (result, gas_info) =
        data.with_querier_from_context::<_, _>(|querier| {
...
>(
    ...
    let start =
        maybe_read_region(&data.memory(&store), start_ptr, MAX_LENGTH_DB_KEY)?;
    let end =
        maybe_read_region(&data.memory(&store), end_ptr, MAX_LENGTH_DB_KEY)?;
...
>(
    ...
    let (key, value) =
        result?.unwrap_or_else(|| (Vec::<u8>::new(), Vec::<u8>::new()));
...
>(
    ...
    let (result, gas_info) =
        data.with_storage_from_context::<_, _>(|store| {
            Ok(store.next_value(iterator_id))
        })?;
...
mod tests {
    use super::*;
    use cosmwasm_std::{
        coins, from_json, AllBalanceResponse, BankQuery, Binary, Empty,
        QueryRequest, SystemError, SystemResult, WasmQuery,
    };
    use hex_literal::hex;
    use std::ptr::NonNull;
    use wasmer::{
        imports, Function, FunctionEnv, Instance as WasmerInstance, Store,
    };
    ...
    fn make_instance(
        ...
        let wasmer_instance = Box::from(
            WasmerInstance::new(&mut store, &module, &import_obj).unwrap(),
        );
    ...
    fn do_addr_canonicalize_works() {
        ...
        let source_ptr = write_data(&mut fe_mut, b"foo");
        let dest_ptr =
            create_empty(&instance, &mut fe_mut, api.canonical_length() as u32);

        leave_default_data(&mut fe_mut);

        let api = MockApi::default();
        let res =
            do_addr_canonicalize(fe_mut.as_mut(), source_ptr, dest_ptr).unwrap();
        assert_eq!(res, 0);
    ...
    fn do_addr_canonicalize_reports_invalid_input_back_to_contract() {
        ...
        let res = do_addr_canonicalize(fe_mut.as_mut(), source_ptr1, dest_ptr)
            .unwrap();
        assert_ne!(res, 0);
        let err = String::from_utf8(force_read(&mut fe_mut, res)).unwrap();
        assert_eq!(err, "Input is not valid UTF-8");

        let res = do_addr_canonicalize(fe_mut.as_mut(), source_ptr2, dest_ptr)
            .unwrap();
        assert_ne!(res, 0);
        let err = String::from_utf8(force_read(&mut fe_mut, res)).unwrap();
        assert_eq!(err, "Input is empty");

        let res = do_addr_canonicalize(fe_mut.as_mut(), source_ptr3, dest_ptr)
            .unwrap();
        assert_ne!(res, 0);
        let err = String::from_utf8(force_read(&mut fe_mut, res)).unwrap();
        assert_eq!(
    ...
    fn do_addr_humanize_works() {
        ...
        let error_ptr =
            do_addr_humanize(fe_mut.as_mut(), source_ptr, dest_ptr).unwrap();
    ...
    fn do_query_chain_works() {
        ...
        let query_result: cosmwasm_std::QuerierResult =
            cosmwasm_std::from_json(response).unwrap();
        ...
        let parsed_again: AllBalanceResponse =
            from_json(query_result_inner_inner).unwrap();
    ...
    fn do_db_scan_unbound_descending_works() {
        ...
        // set up iterator over all space
        let id =
            do_db_scan(fe_mut.as_mut(), 0, 0, Order::Descending.into()).unwrap();
        assert_eq!(1, id);
    ...
    fn do_db_scan_bound_works() {
        ...
        let id =
            do_db_scan(fe_mut.as_mut(), start, end, Order::Ascending.into())
                .unwrap();
