...
use crate::errors::{
    ...
    RegionValidationResult, VmResult,
};
...
pub fn read_region(
    ...
    memory
        ...
        .map_err(|_err| {
            CommunicationError::region_access_err(
                region,
                memory.size().bytes().0,
            )
        })?;
...
pub fn write_region(
    ...
    if data.len() > region_capacity {
        return Err(CommunicationError::region_too_small(
            region_capacity,
            data.len(),
        )
        .into());
    ...
    memory.write(region.offset as u64, data).map_err(|_err| {
        CommunicationError::region_access_err(region, memory.size().bytes().0)
    })?;
...
fn get_region(
    ...
    let region =
        Region::from_wasm_bytes(wptr.deref(memory).read().map_err(|_err| {
