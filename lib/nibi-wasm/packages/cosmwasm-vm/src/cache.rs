...
use crate::modules::{
    CachedModule, FileSystemCache, InMemoryCache, PinnedMemoryCache,
};
    ...
    Q: Querier + 'static,    // 'static is needed by `impl<…> Instance`
{
    ...
    pub unsafe fn new(options: CacheOptions) -> VmResult<Self> {
        ...
        mkdir_p(&state_path).map_err(|_e| {
            VmError::cache_err("Error creating state directory")
        })?;
        mkdir_p(&cache_path).map_err(|_e| {
            VmError::cache_err("Error creating cache directory")
        })?;
        mkdir_p(&wasm_path)
            .map_err(|_e| VmError::cache_err("Error creating wasm directory"))?;

        let fs_cache = FileSystemCache::new(cache_path.join(MODULES_DIR), false)
            .map_err(|e| {
                VmError::cache_err(format!("Error file system cache: {e}"))
            })?;
        Ok(Cache {
            available_capabilities,
    ...
    pub fn pin(&self, checksum: &Checksum) -> VmResult<()> {
        ...
        {
            cache.stats.hits_fs_cache =
                cache.stats.hits_fs_cache.saturating_add(1);
    ...
    fn get_module(&self, checksum: &Checksum) -> VmResult<(Module, Store)> {
        ...
        if let Some(element) = cache.memory_cache.load(checksum)? {
            cache.stats.hits_memory_cache =
                cache.stats.hits_memory_cache.saturating_add(1);
            let CachedModule {
                module,
                engine,
                size_estimate: _,
    ...
    let mut file = OpenOptions::new()
        ...
        .map_err(|e| {
            VmError::cache_err(format!(
                "Error opening Wasm file for writing: {e}"
            ))
        })?;
    file.write_all(wasm).map_err(|e| {
        VmError::cache_err(format!("Error writing Wasm file: {e}"))
    })?;

    Ok(checksum)
}

fn load_wasm_from_disk(
    dir: impl Into<PathBuf>,
    checksum: &Checksum,
) -> VmResult<Vec<u8>> {
    ...
    let mut file = File::open(path.with_extension("wasm"))
        ...
        .map_err(|_e| {
            VmError::cache_err("Error opening Wasm file for reading")
        })?;
...
fn remove_wasm_from_disk(
    ...
    if path_exists {
        fs::remove_file(path).map_err(|_e| {
            VmError::cache_err("Error removing Wasm file from disk")
        })?;
    }

    if wasm_path_exists {
        fs::remove_file(wasm_path).map_err(|_e| {
            VmError::cache_err("Error removing Wasm file from disk")
        })?;
    }
...
mod tests {
    ...
    use crate::testing::{
        mock_backend, mock_env, mock_info, MockApi, MockQuerier, MockStorage,
    };
    ...
    fn test_hackatom_instance_execution<A, S, Q>(
        ...
        Q: Querier + 'static,
    {
        ...
        let msg = br#"{"release":{}}"#;
        let response =
            call_execute::<_, _, _, Empty>(instance, &mock_env(), &info, msg)
                .unwrap()
                .unwrap();
    ...
    fn new_base_dir_will_be_created() {
        let my_base_dir =
            TempDir::new().unwrap().keep().join("non-existent-sub-dir");
        ...
        let _cache = unsafe {
            Cache::<MockApi, MockStorage, MockQuerier>::new(options).unwrap()
        };
        assert!(my_base_dir.is_dir());
    }
    ...
    fn get_instance_recompiles_module() {
        ...
        remove_dir_all(options.base_dir.join(CACHE_DIR).join(MODULES_DIR))
            .unwrap();
    ...
    fn call_execute_on_cached_contract() {
        ...
        // from file system
        {
            ...
            let info = mock_info("verifies", &coins(15, "earth"));
            let msg = br#"{"release":{}}"#;
            let response = call_execute::<_, _, _, Empty>(
                &mut instance,
                &mock_env(),
                &info,
                msg,
    ...
    fn use_multiple_cached_instances_of_same_contract() {
        ...
        let msg = br#"{"release":{}}"#;
        let res = call_execute::<_, _, _, Empty>(
            &mut instance,
            &mock_env(),
            &info,
            msg,
    ...
    fn recovers_from_out_of_gas() {
        ...
        let mut instance1 =
            cache.get_instance(&checksum, backend1, options).unwrap();
        assert_eq!(cache.stats().hits_fs_cache, 1);
        assert_eq!(cache.stats().misses, 0);
        ...
        let mut instance2 =
            cache.get_instance(&checksum, backend2, options).unwrap();
        assert_eq!(cache.stats().hits_pinned_memory_cache, 0);
        assert_eq!(cache.stats().hits_memory_cache, 1);
        assert_eq!(cache.stats().hits_fs_cache, 1);
        assert_eq!(cache.stats().misses, 0);
