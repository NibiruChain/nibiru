...
impl VmError {
    ...
    pub(crate) fn deserialization_limit_exceeded(
        length: usize,
        max_length: usize,
    ) -> Self {
...
mod tests {
    ...
    fn backend_err_works() {
        let error =
            VmError::backend_err(BackendError::unknown("something went wrong"));
        match error {
            VmError::BackendErr {
    ...
    #[test]
    fn cyrpto_err_works() {
        let error = VmError::crypto_err(CryptoError::generic_err(
    ...
    fn resolve_err_works() {
        ...
        match error {
            VmError::ResolveErr { msg, .. } => {
                assert_eq!(msg, "function has different signature")
            }
            e => panic!("Unexpected error: {e:?}"),
        }
    }
    ...
    fn static_validation_err_works() {
        ...
        match error {
            VmError::StaticValidationErr { msg, .. } => {
                assert_eq!(msg, "export xy missing")
            }
            e => panic!("Unexpected error: {e:?}"),
        }
    }
