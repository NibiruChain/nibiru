...
type ContractAddresses struct {
	...
	PerpAddress      string
	VaultAddress     string
	SaiEvmAddress    string
	TokenStNIBIERC20 string // legacy; TOKEN_STNIBI
	TokenUSDCErc20   string // TOKEN_USDC
	StNIBIDenom      string // legacy; STNIBI
...
func loadContractAddresses(envFile string) (ContractAddresses, error) {
	...
	for _, line := range lines {
		...
		case "SAI_EVM_ADDRESS":
			addrs.SaiEvmAddress = val
		case "TOKEN_USDC":
			addrs.TokenUSDCErc20 = val
		case "USDC":
			addrs.USDCDenom = val
		case "TOKEN_STNIBI":
...
func setConfigDefaults(cfg *Config) {
	...
	if cfg.ContractsEnvFile == "" {
		candidates := []string{
			filepath.Join("scripts", "e2e", ".cache", "localnet_contracts.env"),
			filepath.Join(".cache", "localnet_contracts.env"),
			"localnet_contracts.env",
		}
		for _, path := range candidates {
			if _, err := os.Stat(path); err == nil {
				cfg.ContractsEnvFile = path
				return
			}
		}
		cfg.ContractsEnvFile = candidates[0]
	}
}
