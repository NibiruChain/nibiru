...
Technical specification for Nibiru Chain's governance module, `x/gov`. The
...
a 1 token 1 vote basis. {synopsis}
...
* **Claiming deposit:** Users that deposited on proposals can recover their
  deposits if the proposal was accepted or rejected. If the proposal was vetoed,
  or never entered voting period, the deposit is burned.

## Contents

The following specification uses NIBI as the native staking token. The module
can be adapted to any Proof-Of-Stake blockchain by replacing NIBI with the native
...
- [Contents](#contents)
- [Concepts](#concepts)
  - [Concepts: Proposal submission](#concepts-proposal-submission)
  - [Concepts: Deposit](#concepts-deposit)
  - [Concepts: Vote](#concepts-vote)
  - [Concepts: Quorum](#concepts-quorum)
- [State](#state)
  - [State: Proposals](#state-proposals)
  - [State: Parameters and base types](#state-parameters-and-base-types)
  - [State: Deposit](#state-deposit)
  - [State: ValidatorGovInfo](#state-validatorgovinfo)
- [Stores](#stores)
- [Messages](#messages)
...
```bash
nibid query gov deposit [proposal-id] [depositer-addr] [flags]
```

Example:

```bash
nibid query gov deposit 1 cosmos1..
```
...
```bash
nibid query gov deposits [proposal-id] [flags]
```

Example:

```bash
nibid query gov deposits 1
```
...
```bash
nibid query gov param [param-type] [flags]
```

Example:

```bash
nibid query gov param voting
```
...
```bash
nibid query gov params [flags]
```

Example:

```bash
nibid query gov params
```
...
```bash
nibid query gov proposal [proposal-id] [flags]
```
...
```bash
nibid query gov proposals [flags]
```
...
```bash
nibid query gov proposer [proposal-id] [flags]
```

Example:

```bash
nibid query gov proposer 1
```
...
```bash
nibid query gov tally [proposal-id] [flags]
```

Example:

```bash
nibid query gov tally 1
```
...
```bash
nibid query gov vote [proposal-id] [voter-addr] [flags]
```

Example:

```bash
nibid query gov vote 1 cosmos1..
```
...
```bash
nibid query gov votes [proposal-id] [flags]
```

Example:

```bash
nibid query gov votes 1
```
...
```bash
nibid tx gov deposit 1 10000000stake --from nibi1..
```
...
```bash
nibid tx gov submit-proposal [path-to-proposal-json] [flags]
```

Example:

```bash
nibid tx gov submit-proposal /path/to/proposal.json --from nibi1..
```
...
```bash
nibid tx gov submit-legacy-proposal [command] [flags]
```

Example:

```bash
nibid tx gov submit-legacy-proposal --title="Test Proposal" --description="testing" --type="Text" --deposit="100000000stake" --from nibi1..
```

Example (`cancel-software-upgrade`):

```bash
nibid tx gov submit-legacy-proposal cancel-software-upgrade --title="Test Proposal" --description="testing" --deposit="100000000stake" --from nibi1..
```

Example (`community-pool-spend`):

```bash
nibid tx gov submit-legacy-proposal community-pool-spend proposal.json --from nibi1..
```
...
```bash
nibid tx gov submit-legacy-proposal param-change proposal.json --from nibi1..
```
...
```bash
nibid tx gov submit-legacy-proposal software-upgrade v2 --title="Test Proposal" --description="testing, testing, 1, 2, 3" --upgrade-height 1000000 --from nibi1..
```
...
```bash
nibid tx gov vote [command] [flags]
```

Example:

```bash
nibid tx gov vote 1 yes --from nibi1..
```
...
```bash
nibid tx gov weighted-vote [proposal-id] [weighted-options] [flags]
```

Example:

```bash
nibid tx gov weighted-vote 1 yes=0.5,no=0.5 --from nibi1..
```
...
Here we provide a recommendation for the json structure and where the data should
be stored. There are two important factors in making these recommendations.
...
applications such as block explorers and governance interfaces have confidence in
the consistency of metadata structure accross chains.
...
to be listed in the metadata. In v0.46, the `authors` field is a comma-separated
...
* **`BountyProposals`:** If accepted, a `BountyProposal` creates an open bounty.
  ...
  accepted by governance, anybody can submit a `SoftwareUpgradeProposal` with the
  code to claim the bounty. Note that once a `BountyProposal` is accepted, the
  corresponding funds in the `reserve pool` are locked so that payment can always
  be honored. In order to link a `SoftwareUpgradeProposal` to an open bounty, the
  submitter of the `SoftwareUpgradeProposal` will use the
...
* **Complex delegation:** Delegators could choose other representatives than
  ...
  representative before they inherit the vote of their validator. In other words,
  they would only inherit the vote of their validator if their other appointed
