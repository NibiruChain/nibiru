<template>
  <div>
    <transition name="overlay" appear>
      <div class="overlay" ref="overlay" :style="{
        'background-color': backgroundColor || 'rgba(16, 23, 32, 0.8)',
      }" v-if="visible && visibleLocal" @click="close" @touchstart="touchstart" @touchmove="touchmove"
        @touchend="touchend"></div>
    </transition>
    <transition name="sidebar" @after-leave="$emit('visible', false)" appear>
      <div class="sidebar" ref="sidebar" v-if="visible && visibleLocal" :style="style" @touchstart="touchstart"
  ...
  computed: {
    style() {
      return {
        ...
        "--translate-x-component-internal": `${this.side === "right" ? "" : "-"
          }${this.touchMoveX}%`,
      }
    },
  },
