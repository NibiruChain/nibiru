...
<template lang="pug">
  div
    noscript
      iframe(title="Google Tag Manager" src="https://www.googletagmanager.com/ns.html?id=GTM-MTZCQ3SH" height="0" width="0" style="display: none; visibility: hidden")
    <!-- cookie-banner -->
    tm-top-banner(v-if="{topBanner}" v-bind="{topBanner}")
    .layout
      .layout__sidebar
      ...
      .layout__main
        ...
        .layout__main__footer
          tm-footer(:tree="directoryTree" :full="!($page.frontmatter.footer) || $page.frontmatter.footer.newsletter !== true")
    ...
    tm-sidebar(:visible="searchPanel" @visible="searchPanel = $event" max-width="100vw" width="480px" side="right" box-shadow="0 0 50px 10px rgba(0,0,0,.1)" background-color="rgba(0,0,0,0)").sheet__sidebar
      section-search(@visible="searchPanel = $event" :base="$site.base" @cancel="searchPanel = false" :algoliaConfig="algoliaConfig" @select="searchSelect($event)" :query="searchQuery" @query="searchQuery =$event" :site="$site" class="tm-sidebar__section-search")
...
.sheet
  &__sidebar
    z-index 10000
    position relative
    scrollbar-color --var(color-bg) #101720
...
.sidebar .tm-sidebar__section-search .container
  background-color var(--color-bg)
  color var(--color-text)
  --accent-color: var(--color-text)

.sidebar .tm-sidebar__section-search .results__item
  color var(--color-text)
  --accent-color: var(--color-text)
...
.sidebar .tm-sidebar__section-search .shortcuts__table__row__desc
  color: var(--color-text)
.sidebar .tm-sidebar__section-search input
  color: var(--color-text)
  &::placeholder
    color: var(--color-sidebar-dim)

.sidebar .tm-sidebar__section-search .shortcuts__h1
  color: var(--color-text)

.sidebar .tm-sidebar__section-search .search-box
  color var(--color-text)
  --accent-color: var(--color-text)
.sidebar .tm-sidebar__section-search .search-box__input
  color var(--color-text)
  --accent-color: var(--color-text)
...
import { initializeDefaultTagManagerObjects } from "@nibiruchain/ui-nibi"
...
function sendToGoogleAnalytics({ name, delta, value, id }) {
  ...
  const dataLayer = initializeDefaultTagManagerObjects()
  dataLayer.push({ event: name, ...eventParams })
}
...
export default {
  ...
  data: function () {
    return {
      ...
      searchQuery: null,
      prereq: null,
      asideBannersUrl: "https://discord.gg/nibirufi",
      topBannerUrl: "https://x.com/NibiruChain",
      asideBanners: null,
      topBanner: null,
      heightBanners: null,
      status: null,
  ...
  mounted() {
    ...
    document.documentElement.style.setProperty("background-color", `#101720`)
  },
  computed: {
    algoliaConfig() {
