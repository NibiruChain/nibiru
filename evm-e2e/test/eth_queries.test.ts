...
describe('eth queries', () => {
  ...
  it('eth_feeHistory', async () => {
    ...
    const feeHistory = await provider.send('eth_feeHistory', [blockCount, newestBlock, rewardPercentiles]);
  ...
  it('eth_getBlockByNumber, eth_getBlockByHash', async () => {
    ...
    const blockByHash = await provider.send('eth_getBlockByHash', [blockByNumber.hash, false]);
  ...
  it('eth_getProof', async () => {
    ...
    const storageKey = keccak256(AbiCoder.defaultAbiCoder().encode(['address', 'uint256'], [account.address, slot]));
    const proof = await provider.send('eth_getProof', [contractAddr, [storageKey], 'latest']);
  ...
  it('eth_getTransactionByBlockHashAndIndex, eth_getTransactionByBlockNumberAndIndex', async () => {
    ...
    const txByBlockHash = await provider.send('eth_getTransactionByBlockHashAndIndex', [block.hash, '0x0']);
    ...
    const txByBlockNumber = await provider.send('eth_getTransactionByBlockNumberAndIndex', [block.number, '0x0']);
  ...
  it('eth_getUncleCountByBlockHash', async () => {
    ...
    const uncleCount = await provider.send('eth_getUncleCountByBlockHash', [block.hash]);
    expect(parseInt(uncleCount)).toBeGreaterThanOrEqual(0);
  });

  it('eth_getUncleCountByBlockNumber', async () => {
    const latestBlock = await provider.getBlockNumber();
    const uncleCount = await provider.send('eth_getUncleCountByBlockNumber', [latestBlock]);
    expect(parseInt(uncleCount)).toBeGreaterThanOrEqual(0);
  });
