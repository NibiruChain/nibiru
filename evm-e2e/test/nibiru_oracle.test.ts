...
test(
  ...
  async () => {
    const { oraclePair, contract } =
      await deployContractNibiruOracleChainLinkLike()
    ...
    expect(description).toEqual(
      `Nibiru Oracle ChainLink-like price feed for ${oraclePair}`,
    )
    ...
    {
      const { roundId, answer, startedAt, updatedAt, answeredInRound } =
        await contract.getRoundData(0n)
      expect(roundId).toEqual(0n) // price is from genesis block
      expect(startedAt).toBeGreaterThan(1n)
      expect(updatedAt).toBeGreaterThan(1n)
      expect(answeredInRound).toEqual(420n)
