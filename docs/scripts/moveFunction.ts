...
function replaceOriginalWithTemp(tempPath: string) {
  ...
  const outFilePath: string = path.join(...PATH_LIST_DIR.concat(["exports.md"]))
...
async function addTitleToReadme() {
  ...
  let linesReadme: string[] = fs.readFileSync(pathReadme).toString().split("\n")
...
function changeExportsTitle() {
  const pathExports: string = path.join(...PATH_LIST_DIR.concat(["exports.md"]))
