...
REPO_DIR=$(pwd)
if [ "$(basename "$PWD")" != "docs-v2" ]; then
  echo "pwd: $(pwd)"
  echo "Please run the script from docs-v2."
  exit 1
fi
...
# From inside the NEW_DOCS_DIR, create index.md and nibijs.md
...
cp ts-sdk/packages/nibijs/README.md $NEW_DOCS_DIR/index.md
cd $NEW_DOCS_DIR
rm intro.md
egrep -lRZ 'intro.md' . | xargs -0 -l sed -i -e 's/intro.md/index.md/g'
...
egrep -lRZ 'nibijs.md' $NEW_DOCS_DIR/classes | xargs -0 -l sed -i -e 's/nibijs.md/index.md/g'
egrep -lRZ 'nibijs.md' $NEW_DOCS_DIR/enums | xargs -0 -l sed -i -e 's/nibijs.md/index.md/g'
egrep -lRZ 'nibijs.md' $NEW_DOCS_DIR/interfaces | xargs -0 -l sed -i -e 's/nibijs.md/index.md/g'
