...
import (
	...
	"github.com/NibiruChain/nibiru/v2/evm/rpc/pubsub"
	"github.com/NibiruChain/nibiru/v2/x/nutil/testutil"
)

func TestEventSubscriber(t *testing.T) {
	index := make(jsonrpc.FilterIndex)
	for i := filters.UnknownSubscription; i < filters.LastIndexSubscription; i++ {
		index[i] = make(map[rpc.ID]*jsonrpc.Subscription)
	}
	es := &jsonrpc.EventSubscriber{
		...
		Install:    make(chan *jsonrpc.Subscription),
		Uninstall:  make(chan *jsonrpc.Subscription),
		EventBus:   pubsub.NewEventBus(),
	}
	go es.EventLoop()

	event := "event"
	sub := jsonrpc.MakeSubscription("1", event)
	es.Install <- sub
	<-sub.Installed
	...
	sub = jsonrpc.MakeSubscription("2", event)
	es.Install <- sub
	<-sub.Installed
...
func TestEventSubscriberUninstallKeepsSharedTopic(t *testing.T) {
	...
	event := "shared.event"
	sub1 := jsonrpc.MakeSubscription("1", event)
	sub2 := jsonrpc.MakeSubscription("2", event)
	es.Install <- sub1
	<-sub1.Installed
...
func (s *Suite) TestParseBloomFromEvents() {
	for _, tc := range []struct {
		...
		s.Run(tc.name, func() {
			...
			bloom, err := jsonrpc.ParseBloomFromEvents(events)
...
func newTestEventSubscriber() *jsonrpc.EventSubscriber {
	index := make(jsonrpc.FilterIndex)
	for i := filters.UnknownSubscription; i < filters.LastIndexSubscription; i++ {
		index[i] = make(map[rpc.ID]*jsonrpc.Subscription)
	}

	return &jsonrpc.EventSubscriber{
		Logger:     log.NewTMLogger(log.NewSyncWriter(os.Stdout)),
		Ctx:        context.Background(),
		LightMode:  false,
		Index:      index,
