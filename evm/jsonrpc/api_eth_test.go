...
func (s *Suite) TestExpectedMethods() {
	...
	apis := jsonrpc.GetRPCAPIs(
		...
		true, nil,
		[]string{
			jsonrpc.NamespaceEth, // eth and filters services
	...
	testCases := []TestCase{
		...
		{
			ServiceName: "jsonrpc.FiltersAPI",
			Methods: []string{
	...
	for idx, api := range apis {
		...
		s.Run(testName, func() {
			gotMethods := jsonrpc.ParseAPIMethods(api)
...
func (s *NodeSuite) waitForEthReceipt(txHash gethcommon.Hash) (*jsonrpc.TransactionReceipt, error) {
	var lastErr error
